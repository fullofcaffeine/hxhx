...
class M14CppIntModInt64IntegrationTest {
	...
	static function main():Void {
		final sourcePath = "test/oracle/cpp_int_mod_int64_seed/src/Main.hx";
		...
		final int64Source = [
			...
			"  @:op(A % B) private static inline function intMod(dividend:Int, divisor:Int64):Int64 return divisor;",
			"}",
		].join("\n");
		...
		final root = Path.join([Sys.getCwd(), ".tmp", "m14_cpp_int_mod_int64"]);
		...
		assertTrue(generated.indexOf("Int64::intMod") < 0, "C++ emitted a nonexistent Int64.intMod helper");
		assertTrue(generated.split("__hxhx_int64_mod(").length > 6, "C++ did not route Int % Int64 through the checked carrier helper");
		final orderedCall = new EReg("__hxhx_int64_mod\\(__hxhx_abstract_binary_left_[0-9]+, __hxhx_abstract_binary_right_[0-9]+\\)", "");
		final reversedCall = new EReg("__hxhx_int64_mod\\(__hxhx_abstract_binary_right_[0-9]+, __hxhx_abstract_binary_left_[0-9]+\\)", "");
		assertTrue(orderedCall.match(generated) && !reversedCall.match(generated), "C++ did not preserve Int % Int64 operand order");
		final rawCarrierMod = new EReg("__hxhx_abstract_binary_left_[0-9]+ % __hxhx_abstract_binary_right_[0-9]+", "");
		assertTrue(!rawCarrierMod.match(generated), "C++ retained raw signed remainder for a typed Int64 operator");
		assertTrue(generated.indexOf("if (divisor == 0) throw std::runtime_error(\"divide by zero\")") >= 0,
			"C++ Int64 remainder does not preserve Haxe's divide-by-zero exception");
		assertTrue(generated.indexOf("dividend == std::numeric_limits<long long>::min() && divisor == -1) return 0") >= 0,
			"C++ Int64 remainder does not guard the minimum-value modulo negative-one boundary");
		assertTrue(generated.indexOf("__hxhx_int64_to_int(__hxhx_int64_mod(") < 0, "C++ invented 32-bit narrowing for Int % Int64");
		assertTrue(generated.indexOf("static_cast<int>(__hxhx_int64_mod(") < 0, "C++ cast the Int % Int64 remainder back to Int");
		...
		final expected = File.getContent("test/oracle/cpp_int_mod_int64_seed/expected.stdout");
