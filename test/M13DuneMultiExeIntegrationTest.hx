class M13DuneMultiExeIntegrationTest {
	...
	static function main() {
		...
		final args = [
			...
			"ocaml_output=" + outDir,
			"-D",
			"ocaml_dune_exes=foo:pkg.M13MliMain,bar:pkg.M13MliMain"
		];
		...
		if (!sys.FileSystem.exists(fooPath))
			throw "missing entry module: " + fooPath;
		if (!sys.FileSystem.exists(barPath))
			throw "missing entry module: " + barPath;
