...
class OcamlASTPrinterTest {
	...
	static function main() {
		...
		assertEq(
			...
			p.printItem(OcamlModuleItem.IType([{
				name: "t",
				params: [],
		...
		try {
			...
			if (ocamlc != null) {
				...
				try sys.FileSystem.deleteFile(tmpDir + "/PrinterCheck.cmi") catch (_:Dynamic) {}
				try sys.FileSystem.deleteFile(tmpDir + "/PrinterCheck.cmo") catch (_:Dynamic) {}
				try sys.FileSystem.deleteDirectory(tmpDir) catch (_:Dynamic) {}
			}
		} catch (_:Dynamic) {}
	}
