...
/** Compares Python Boolean string conversion with upstream Haxe. */
class M14PythonBoolStringIntegrationTest {
	...
	static function main():Void {
		final fixture = "test/python_bool_string";
		final expected = File.getContent(fixture + "/expected.stdout");
		if (run("haxe", ["-cp", fixture, "--run", "Main"]) != expected)
			throw "upstream Python Boolean strings contract differs from its authored expectation";
		...
		final root = Path.normalize(Sys.getCwd() + "/.tmp/python_bool_string_" + Std.string(Date.now().getTime()));
		...
		if (actual != expected)
			throw "native Python Boolean strings output mismatch; artifacts: " + root;
		removeTree(root);
		Sys.println("PYTHON_BOOL_STRING:PASS");
	}
}
