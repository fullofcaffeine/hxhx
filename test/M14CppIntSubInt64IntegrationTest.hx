...
class M14CppIntSubInt64IntegrationTest {
	...
	static function main():Void {
		final sourcePath = "test/oracle/cpp_int_sub_int64_seed/src/Main.hx";
		...
		final int64Source = [
			...
			"  @:op(A - B) private static inline function intSub(left:Int, right:Int64):Int64 return right;",
			"}",
		].join("\n");
		...
		final root = Path.join([Sys.getCwd(), ".tmp", "m14_cpp_int_sub_int64"]);
		...
		assertTrue(generated.indexOf("Int64::intSub") < 0, "C++ emitted a nonexistent Int64.intSub helper");
		...
		final orderedCall = new EReg("__hxhx_int64_sub\\(__hxhx_abstract_binary_left_[0-9]+, __hxhx_abstract_binary_right_[0-9]+\\)", "");
		final reversedCall = new EReg("__hxhx_int64_sub\\(__hxhx_abstract_binary_right_[0-9]+, __hxhx_abstract_binary_left_[0-9]+\\)", "");
		...
		final expected = File.getContent("test/oracle/cpp_int_sub_int64_seed/expected.stdout");
