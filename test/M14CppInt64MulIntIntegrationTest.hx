...
/** Builds and executes the repo-owned commutative `Int64 * Int` contract through C++. **/
class M14CppInt64MulIntIntegrationTest {
	...
	static function main():Void {
		final sourcePath = "test/oracle/cpp_int64_mul_int_seed/src/Main.hx";
		...
		final int64Source = [
			...
			"  @:op(A * B) @:commutative private static inline function mulInt(value:Int64, factor:Int):Int64 return value;",
			"}",
		].join("\n");
		...
		final root = Path.join([Sys.getCwd(), ".tmp", "m14_cpp_int64_mul_int"]);
		...
		assertTrue(generated.indexOf("Int64::mulInt") < 0, "C++ emitted a nonexistent Int64.mulInt helper");
		assertTrue(generated.indexOf("Int64::mul(") < 0, "C++ emitted Int64.mul instead of the native carrier helper");
		assertTrue(generated.split("__hxhx_int64_mul(").length > 3, "C++ did not route Int64 multiplication through the wrapping carrier helper");
		final orderedCall = new EReg("__hxhx_int64_mul\\(__hxhx_abstract_binary_left_[0-9]+, __hxhx_abstract_binary_right_[0-9]+\\)", "");
		final reversedCall = new EReg("__hxhx_int64_mul\\(__hxhx_abstract_binary_right_[0-9]+, __hxhx_abstract_binary_left_[0-9]+\\)", "");
		assertTrue(orderedCall.match(generated) && reversedCall.match(generated), "C++ did not preserve both commutative Int64 * Int call shapes");
		final rawCarrierMul = new EReg("__hxhx_abstract_binary_(left|right)_[0-9]+ \\* __hxhx_abstract_binary_(left|right)_[0-9]+", "");
		assertTrue(!rawCarrierMul.match(generated), "C++ retained raw signed multiplication for a typed Int64 operator");
		assertTrue(generated.indexOf("static_cast<unsigned long long>(left) * static_cast<unsigned long long>(right)") >= 0,
			"C++ Int64 multiplication does not make modulo arithmetic explicit");
		...
		final expected = File.getContent("test/oracle/cpp_int64_mul_int_seed/expected.stdout");
