...
let init () : unit =
  ...
  HxType.register_class_ctor "Point" (fun (args : Obj.t HxArray.t) ->
    let len = HxArray.length args in
    let a0 = if len > 0 then Obj.obj ((HxArray.get args 0)) else failwith "Type.createInstance: missing ctor arg 'x' for Point" in
    let a1 = if len > 1 then Obj.obj ((HxArray.get args 1)) else failwith "Type.createInstance: missing ctor arg 'y' for Point" in
    Obj.repr (Point.create a0 a1)
  );
  ...
  HxType.register_class_empty_ctor "Point" (fun () -> Obj.repr (Point.__empty ()));
  ...
  HxType.register_class_instance_fields "Point" [ "add"; "incX"; "sum"; "x"; "y" ];
  HxType.register_class_static_fields "Point" [];
