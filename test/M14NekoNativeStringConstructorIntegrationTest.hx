...
/**
	Loads the real Neko String provider and compares native construction with
	upstream execution. A qualified ordinary String class retains object behavior.
**/
class M14NekoNativeStringConstructorIntegrationTest {
	...
	static function main():Void {
		final fixture = "test/neko_native_string_constructor";
		final root = ".tmp/neko_native_string_constructor_" + Date.now().getTime();
		...
		if (run("neko", [root + "/upstream.n"]) != expected)
			throw "upstream String constructor result changed";
		...
		var cursor = 0;
		var foundNativeString = false;
		while (cursor < pending.length) {
			final module = pending[cursor++];
			if (ResolvedModule.getModulePath(module) == "String") {
				final provider = ResolvedModule.getFilePath(module).split("\\").join("/");
				if (!StringTools.endsWith(provider, "/neko/_std/String.hx"))
					throw "fixture selected the wrong String provider: " + provider;
				foundNativeString = true;
			}
			typed.push(TyperStage.typeResolvedModule(module, index, loader, true));
			for (loaded in loader.drainNewModules())
				pending.push(loaded);
		}
		if (!foundNativeString)
			throw "real Neko String provider was not loaded";
		...
		final emitted = single + [for (part in split.support) part.source].join("\n");
		if (emitted.indexOf(".__hxhx_new_String =") >= 0)
			throw "native String acquired an object constructor";
		if (emitted.indexOf(".__hxhx_new_ordinary_String =") < 0)
			throw "qualified String lost its ordinary constructor";
		...
		for (layout in ["main", "single"]) {
			...
			if (actual != expected)
				throw "String constructor differs in " + layout + ": " + root;
			Sys.println("NEKO_NATIVE_STRING_CONSTRUCTOR:PASS layout=" + layout);
		}
	}
}
