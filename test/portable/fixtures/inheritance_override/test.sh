...
# The upstream routes establish the observable dispatch and return behavior.
# The native build performed by the portable harness is checked below for the
# narrower compiler claim: each concrete method owns only its completed String
# value and does not gain a direct receiver, dispatch, or call contract.
bash ../../../../scripts/reflaxe-ocaml/run-inheritance-override-oracle.sh

node - out/Main.ml out/haxe_Exception.ml out/ocaml_lowering_report.json <<'NODE'
const fs = require('fs')
...
if (report.schemaVersion !== 63 || stringResults.length !== expected.length) {
...
if (report.schemaVersion !== 40
	...
	throw new Error('public inspection did not preserve the three declaration-only instance String results')
}
NODE
...
echo "INHERITANCE_STRING_RESULT_BOUNDARY:PASS"
