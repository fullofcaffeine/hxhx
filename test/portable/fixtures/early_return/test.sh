...
if (nestedThrows.length !== 1
	...
	|| nestedThrows[0].pipelineRevision !== 'ocaml-nested-function-plans-v28') {
...
if (nestedCatches.length !== 2
	|| nestedCatches.some(catchChain =>
		catchChain.pipelineRevision !== 'ocaml-nested-function-plans-v28'
...
if (nestedLoopTargets.length !== 1
	...
	|| nestedLoopTargets[0].pipelineRevision !== 'ocaml-nested-function-plans-v28'
...
for (const control of returnControls) {
	...
	if (effectOnlyVoid) {
		if (ids.has(control.id)
			...
			|| control.pipelineRevision !== 'ocaml-function-plans-v107') {
	...
	if (ids.has(control.id)
		...
		|| !bodyRevision.test(control.bodyRevision)
		|| (control.functionId.includes('|nested-function|')
			? control.pipelineRevision !== 'ocaml-nested-function-plans-v28'
			: control.pipelineRevision !== 'ocaml-function-plans-v107')) {
...
if (dynamicBranchControl?.pipelineRevision !== 'ocaml-function-plans-v107'
