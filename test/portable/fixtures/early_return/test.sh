...
if (nestedThrows.length !== 1
	...
	|| nestedThrows[0].pipelineRevision !== 'ocaml-nested-function-plans-v16') {
...
if (nestedCatches.length !== 2
	|| nestedCatches.some(catchChain =>
		catchChain.pipelineRevision !== 'ocaml-nested-function-plans-v16'
...
if (nestedLoopTargets.length !== 1
	...
	|| nestedLoopTargets[0].pipelineRevision !== 'ocaml-nested-function-plans-v16'
...
for (const control of returnControls) {
	...
	if (effectOnlyVoid) {
		if (ids.has(control.id)
			...
			|| control.pipelineRevision !== 'ocaml-function-plans-v90') {
...
if (dynamicBranchControl?.pipelineRevision !== 'ocaml-function-plans-v90'
