...
assert.equal(report.schemaVersion, 81)
assert.equal(report.callModel, 'typed-ocaml-directional-call-boundary-v29')
assert.equal(
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v100')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v100')
