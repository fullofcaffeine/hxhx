...
assert.equal(report.schemaVersion, 66)
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v78')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v78')
