...
assert.equal(report.schemaVersion, 85)
assert.equal(report.callModel, 'typed-ocaml-directional-call-boundary-v30')
assert.equal(
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v108')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v108')
