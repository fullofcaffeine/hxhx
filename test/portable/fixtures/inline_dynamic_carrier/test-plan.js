...
assert.equal(report.schemaVersion, 68)
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v82')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v82')
