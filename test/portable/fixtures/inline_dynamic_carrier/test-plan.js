...
assert.equal(report.schemaVersion, 72)
assert.equal(report.callModel, 'typed-ocaml-directional-call-boundary-v21')
assert.equal(
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v87')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v87')
