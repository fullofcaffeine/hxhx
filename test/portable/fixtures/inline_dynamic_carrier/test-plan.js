...
assert.equal(report.schemaVersion, 50)
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v63')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v63')
