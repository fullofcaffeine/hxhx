...
assert.equal(report.schemaVersion, 80)
assert.equal(report.callModel, 'typed-ocaml-directional-call-boundary-v28')
assert.equal(
...
for (const operation of dynamicUnsafeOperations) {
	...
	assert.equal(operation.pipelineRevision, 'ocaml-function-plans-v96')
}
...
for (const boundary of dynamicCallableBoundaries) {
	assert.equal(boundary.pipelineRevision, 'ocaml-function-plans-v96')
