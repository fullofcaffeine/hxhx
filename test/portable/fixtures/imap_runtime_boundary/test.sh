...
if (calls.some(call =>
	call.pipelineRevision !== (call.functionId.includes('|nested-function|')
		? 'ocaml-nested-function-plans-v17'
		: 'ocaml-function-plans-v95')
...
if (nestedCalls.length !== 1
	...
	|| nestedCalls[0].pipelineRevision !== 'ocaml-nested-function-plans-v17') {
...
if (nestedConversions.length !== 1
	...
	|| nestedConversions[0].pipelineRevision !== 'ocaml-nested-function-plans-v17') {
...
for (const expected of expectedAliases) {
	...
	const expectedPipeline = expected.nested ? 'ocaml-nested-function-plans-v17' : 'ocaml-function-plans-v95'
	if (!alias
...
nested.pipelineRevision = 'ocaml-function-plans-v95'
report.iMapInterfaceRevision = `sha256:${crypto.createHash('sha256').update(JSON.stringify({
	conversions: report.iMapInterfaceConversions,
	calls: report.iMapInterfaceCalls,
	storageAliases: report.iMapStorageAliases
