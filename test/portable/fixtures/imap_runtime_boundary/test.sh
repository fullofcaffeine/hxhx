...
if (calls.some(call =>
	call.pipelineRevision !== (call.functionId.includes('|nested-function|')
		? 'ocaml-nested-function-plans-v21'
		: 'ocaml-function-plans-v100')
...
if (nestedCalls.length !== 1
	...
	|| nestedCalls[0].pipelineRevision !== 'ocaml-nested-function-plans-v21') {
...
if (nestedConversions.length !== 1
	...
	|| nestedConversions[0].pipelineRevision !== 'ocaml-nested-function-plans-v21') {
...
for (const expected of expectedAliases) {
	...
	const expectedPipeline = expected.nested ? 'ocaml-nested-function-plans-v21' : 'ocaml-function-plans-v100'
	if (!alias
...
nested.pipelineRevision = 'ocaml-function-plans-v100'
report.iMapInterfaceRevision = `sha256:${crypto.createHash('sha256').update(JSON.stringify({
	conversions: report.iMapInterfaceConversions,
	calls: report.iMapInterfaceCalls,
	storageAliases: report.iMapStorageAliases
