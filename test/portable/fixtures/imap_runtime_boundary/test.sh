...
if (report.schemaVersion !== 74
	|| report.callModel !== 'typed-ocaml-directional-call-boundary-v23') {
...
for (const expected of expectedAliases) {
	...
	const expectedPipeline = expected.nested ? 'ocaml-nested-function-plans-v16' : 'ocaml-function-plans-v89'
	if (!alias
...
nested.pipelineRevision = 'ocaml-function-plans-v89'
report.iMapInterfaceRevision = `sha256:${crypto.createHash('sha256').update(JSON.stringify({
	conversions: report.iMapInterfaceConversions,
	calls: report.iMapInterfaceCalls,
	storageAliases: report.iMapStorageAliases
