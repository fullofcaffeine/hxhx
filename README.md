...
| `hxhx` as a MIT Haxe replacement | `[##--------]` | **Not production-ready.** The goal is Haxe `4.3.7` compatibility for the declared Full1 target/generator scope, with no required upstream-Haxe fallback and credible performance including practical edit-compile-test latency. It is not an all-target claim. | Use `hxhx` for scoped native lanes, experiments, and subprocess embedding where the supported scope matches your use case. Read `docs/02-user-guide/compat/FULL1_TARGET_SCOPE.md` before evaluating it for a project. | `haxe.ocaml-f1cl` owns same-candidate outcomes. The target decision is complete: C++/Cppia and both HashLink forms are required; JVM, XML/JSON type descriptions, and Flash/SWF are excluded as documented. Scoped/semantic-version identity (`haxe_ocaml-ftrhr`) and strict suites/targets/macro/plugin/perf remain open. The candidate-bound release handoff has correctly produced only a no-go receipt because real Full1 evidence is incomplete. |
