...
[![Version](https://img.shields.io/badge/version-0.18.18-blue)](https://github.com/fullofcaffeine/hxhx/releases)
...
| --- | --- | --- | --- | --- |
| `reflaxe.ocaml` with upstream `haxe` | `[#######---]` | **Advanced preview.** It is the most usable route here, and a production candidate only for the declared example/runtime matrix after validating your own application. A deterministic source-only ZIP now has an isolated install/native external-app proof. | Use upstream Haxe `4.3.7` plus `-lib reflaxe.ocaml`. Start with `docs/01-getting-started/REFLAXE_OCAML_WITH_UPSTREAM_HAXE.md`. | Active product owner `haxe_ocaml-s7jry` still needs a declared supported-platform matrix, retained same-SHA release provenance, and sustained per-platform performance evidence. The completed foundation is `haxe.ocaml-ro10`; package proof is owned by `haxe_ocaml-s7jry.1`. |
