...
| `reflaxe.ocaml` with `hxhx` | `[##--------]` about 20–25% | **Experimental integration.** Native `--ocaml` uses the standalone target for a small revision-one subset. It accepts selected static final initializers and empty static functions. Ordinary calls and recursive applications remain unsupported. | Use it for native compiler and host integration experiments. Keep upstream Haxe plus standalone `reflaxe.ocaml` as the practical target baseline. | `haxe_ocaml-38gsp.1` published the initial cutover at `6f1d48f52`; runtime requirements and broader target input remain unfinished. `38gsp.2` must then prove two native generations with stage0 forbidden. Full1 and product evidence remain separate gates. |
