...
| `reflaxe.ocaml` as a native `hxhx` plugin | `[###-------]` | **Experimental promotion path.** ABI, registry, promotion workflow, and plugin-safe output primitives exist; broad supported packaging does not. | Reflaxe target authors can use the promotion docs to build and validate native plugin artifacts. | Full1 plugin outcome owner `haxe_ocaml-gskz9` and promotion product owner `haxe_ocaml-bomhr` still need authentic workload, package/install, versioning, provenance, and current-candidate evidence. |
| `reflaxe.ocaml` as a builtin/native `hxhx` target | `[##--------]` | **Architecture proof.** The target-core/adapter design supports a builtin host shape, but it is not productized. | Use the promotion matrix docs to understand plugin versus builtin packaging. | `haxe_ocaml-bomhr` must prove one semantic target core across upstream-host, plugin, and builtin forms with real workload and artifact evidence. Completed foundations: `haxe.ocaml-rpmx` and `haxe.ocaml-anoy`. |
| `hxhx` as a MIT drop-in Haxe replacement | `[##--------]` | **Not production-ready.** The goal is Haxe `4.3.7`-equivalent behavior for an explicit owner-approved compatibility scope, no required upstream-Haxe fallback, and credible performance including practical edit-compile-test latency. | Use `hxhx` for scoped native lanes, experiments, and subprocess embedding where the supported scope matches your use case. | `haxe.ocaml-f1cl` owns same-candidate outcomes. Target scope (`haxe_ocaml-rttuj`), Scoped/semantic-version identity (`haxe_ocaml-ftrhr`), strict suites/targets/macro/plugin/perf, and required-lane green evidence remain open. The prepublication RC/release handoff is now candidate-bound and fail-closed, but it has correctly produced only a no-go receipt because the real Full1 evidence is incomplete. |
| `hxhx` as a hackable Haxe-in-Haxe compiler | `[####------]` | **Active design principle.** The package/phase/backend seams are real, while concentrated implementation and test responsibilities still limit routine reviewability. | Use repo docs, focused tests, and beads to make changes through bounded seams. Keep behavior-driven tests ahead of large rewrites. | Bridge exit criteria (`haxe_ocaml-slobw`) and smoke decomposition (`haxe_ocaml-o2udb`) are active. Broader extraction remains profile/recurrence-triggered; no compiler-wide rewrite or speculative neutral IR is planned. |
| Pluggable Haxe customization and Haxe-family variants | `[#---------]` | **Architecture foundation only.** This is not a supported platform and must remain excluded from baseline evidence. | Use existing macro libraries, Reflaxe targets, and explicit `hxhx` experiments for bounded customization. Keep baseline Haxe compatibility as the default. | Post-Full1 successor `haxe_ocaml-h5jta` owns lifecycle, capability, reversibility, conflict, packaging, and baseline-exclusion evidence. The completed design/toggle foundation is `haxe.ocaml-vary`. |
| Source/native target compilation beyond OCaml | `[####------]` | **Target bring-up.** Several strict/focused target results exist, but the aggregate declared matrix is not green and target obligations are not final. | For OCaml output, use `reflaxe.ocaml` with upstream Haxe. Use the source-target lanes only for validation, experiments, and closing gate blockers. | Scope decision `haxe_ocaml-rttuj` must explicitly disposition every Haxe `4.3.7` target/generator. Matrix owners `haxe.ocaml-f1cl.3.1` and `.3.11` require same-SHA per-target evidence; Cpp leaf `haxe_ocaml-94hk1` remains conditional P2 work. |
