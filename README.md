...
Gate 1 unit-macro rungs fail fast by default with no Darwin-specific retry fallback. `stage3 no-emit` and `stage3 type-only` run with resolver widening enabled (`HXHX_RESOLVE_IMPLICIT_PACKAGE_TYPES=1`), while `stage3 emit` remains pinned to `0`; widened emit-run still has a Darwin native runtime crash path (tracked in `haxe.ocaml-kun`).
