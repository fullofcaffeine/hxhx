...
Readiness review, 2026-09-13: focused exception-transport and runtime-report
...
The supported `--ocaml` route uses the standalone target core for a small revision-one subset.
The fresh PR #43 compiler rejects the reduced cycle example before output: `OCaml target program revision 1 cannot lower function "main"`.
...
| --- | --- | --- | --- | --- |
| `reflaxe.ocaml` with upstream `haxe` | `[#######---]` about 65–70% | **Advanced preview.** This remains the most usable route. The declared example/runtime matrix, deterministic source package, and isolated macOS/Ubuntu native application proofs exist. | Use upstream Haxe `4.3.7` plus `-lib reflaxe.ocaml`. Package `build` and `watch` use a fresh Haxe process by default; explicit local `--connect <port>` is an experimental frontend-reuse lane. Start with `docs/01-getting-started/REFLAXE_OCAML_WITH_UPSTREAM_HAXE.md`. | Representation/storage/capture (`9bome`), calls/conversions (`taef5`), and control effects (`w32h3`) are closed within their admitted scopes. Product owner `haxe_ocaml-s7jry` still needs fail-closed runtime ownership (`0uwin`), complete artifact/native-dependency authority, and fresh release evidence. Runtime ownership now checks typed String equality, direct standard String methods and fields, String null checks in typed `Reflect.compare`, typed String conversion, and seven standard `Reflect` field operations with Haxe's left-to-right evaluation order. Calls to the exact generic identity shape now preserve concrete `String`, `Int`, and `Bool` values without `Obj.t`. However, 209 legacy helper sites remain, so the readiness range is unchanged. Opt-in exact unchanged-request replay has passed compiler-scale qualification, but it is not yet a documented, supported, or default server feature; reuse after edits remains open. |
| `reflaxe.ocaml` with `hxhx` | `[##--------]` about 20–25% | **Experimental integration.** The supported OCaml wrapper now calls the standalone target core for a small subset. It admits primitive locals, blocks, and same-class static calls with no arguments and `Void` results. | Use it for native compiler and host integration experiments. Keep upstream Haxe plus standalone `reflaxe.ocaml` as the practical target baseline. | `haxe_ocaml-38gsp.1` still owns the broader shared-target prerequisites. Arguments, returned values, cross-class calls, and control flow remain unfinished in this subset. `38gsp.2` must prove two native generations with stage0 forbidden. Interpreter-hosted adapter tests and generated application execution do not establish full native compiler acceptance. |
...
| `reflaxe.ocaml` as a builtin/native `hxhx` target | `[##--------]` about 15–20% | **Architecture proof.** The builtin `ocaml-native` adapter invokes the standalone target core for the revision-one subset. The separate `ocaml-stage3` target retains the old emitter for diagnostics. | Use the small shared-target fixtures to inspect native host integration. Use the promotion matrix to understand the intended plugin/builtin composition. | Expand supported inputs and finish the runtime prerequisites of `38gsp.1`. Prove that the plugin and builtin consume the same canonical request/service contract; direct calls may optimize transport only. |
