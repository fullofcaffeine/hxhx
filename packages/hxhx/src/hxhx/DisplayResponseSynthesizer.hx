...
class DisplayResponseSynthesizer {
	...
	static function parseDisplayRequestQuery(displayRequest:String):DisplayRequestQuery {
		if (displayRequest == null)
			return {sourcePath: "", cursorOffset: -1, mode: ""};
		...
		if (at == -1)
			return {sourcePath: trimmed, cursorOffset: -1, mode: ""};
	...
	public static function readDisplaySource(displayRequest:String, stdinBytes:Null<Bytes>):String {
		...
		if (query.sourcePath == null || query.sourcePath.length == 0)
			return "";
		if (!sys.FileSystem.exists(query.sourcePath))
			return "";
		if (sys.FileSystem.isDirectory(query.sourcePath))
			return "";
		return try {
	...
	static function tokenizeDisplaySource(source:String):Array<DisplayToken> {
		...
		while (index < source.length) {
			...
			if (isIdentStart(code)) {
				final start = index;
				index += 1;
				while (index < source.length && isIdentContinue(source.charCodeAt(index)))
					index += 1;
				out.push({
					text: source.substr(start, index - start),
					start: start,
	...
	static function findMatchingCloseToken(tokens:Array<DisplayToken>, openIndex:Int, openSymbol:String, closeSymbol:String):Int {
		if (tokens == null || openIndex < 0 || openIndex >= tokens.length)
			return -1;
		var depth = 0;
	...
	static function extractArgTypeHintFromSegment(source:String, tokens:Array<DisplayToken>, segmentStart:Int, segmentEnd:Int):String {
		...
		for (tokenIndex in segmentStart...segmentEnd) {
			...
			switch (token.text) {
				case "(":
					depthParenOuter += 1;
				case ")":
					if (depthParenOuter > 0)
						depthParenOuter -= 1;
				case "{":
					depthBraceOuter += 1;
				case "}":
					if (depthBraceOuter > 0)
						depthBraceOuter -= 1;
				case "[":
					depthBracketOuter += 1;
				case "]":
					if (depthBracketOuter > 0)
						depthBracketOuter -= 1;
				case "<":
					depthAngleOuter += 1;
				case ">":
					if (depthAngleOuter > 0)
						depthAngleOuter -= 1;
				case ":":
		...
		if (colonIndex < 0 || colonIndex + 1 >= segmentEnd)
			return "";
		...
		for (tokenIndex in typeStartIndex...segmentEnd) {
			...
			switch (token.text) {
				case "(":
					depthParenInner += 1;
				case ")":
					if (depthParenInner > 0)
						depthParenInner -= 1;
				case "{":
					depthBraceInner += 1;
				case "}":
					if (depthBraceInner > 0)
						depthBraceInner -= 1;
				case "[":
					depthBracketInner += 1;
				case "]":
					if (depthBracketInner > 0)
						depthBracketInner -= 1;
				case "<":
					depthAngleInner += 1;
				case ">":
					if (depthAngleInner > 0)
						depthAngleInner -= 1;
				case "=":
		...
		if (startPos < 0 || endPos <= startPos || endPos > source.length)
			return "";
	...
	static function findFunctionArgTypeHint(source:String, tokens:Array<DisplayToken>, functionName:String, argumentIndex:Int):String {
		if (functionName == null || functionName.length == 0 || argumentIndex < 0)
			return "";

		for (tokenIndex in 0...tokens.length) {
			if (!tokens[tokenIndex].isIdent || tokens[tokenIndex].text != "function")
				continue;

			var nameIndex = tokenIndex + 1;
			while (nameIndex < tokens.length && !tokens[nameIndex].isIdent)
				nameIndex += 1;
			if (nameIndex >= tokens.length)
				continue;
			if (tokens[nameIndex].text != functionName)
				continue;

			var openParenIndex = nameIndex + 1;
			while (openParenIndex < tokens.length && tokens[openParenIndex].text != "(")
				openParenIndex += 1;
			...
			for (segmentIndex in (openParenIndex + 1)...(closeParenIndex + 1)) {
				...
				final atTopLevelComma = !boundary && token.text == "," && depthParen == 0 && depthBrace == 0 && depthBracket == 0 && depthAngle == 0;
	...
	static function extractExprOfInner(typeHint:String):String {
		...
		for (index in innerStart...compact.length) {
			...
			if (code == "<".code) {
				...
				depth -= 1;
				if (depth == 0)
					return compact.substr(innerStart, index - innerStart);
			}
		}
	...
	static function findTypedefStructBody(source:String, tokens:Array<DisplayToken>, typeName:String):String {
		...
		for (tokenIndex in 0...tokens.length) {
			final token = tokens[tokenIndex];
			if (!token.isIdent || token.text != "typedef")
				continue;
			...
			if (nameIndex >= tokens.length)
				continue;
			if (tokens[nameIndex].text != simpleName)
				continue;

			var equalsIndex = nameIndex + 1;
			while (equalsIndex < tokens.length && tokens[equalsIndex].text != "=")
				equalsIndex += 1;
			if (equalsIndex >= tokens.length)
				continue;

			var braceIndex = equalsIndex + 1;
			while (braceIndex < tokens.length && tokens[braceIndex].text != "{")
				braceIndex += 1;
			...
			if (endPos <= startPos || endPos > source.length)
				continue;
	...
	static function synthesizeExprOfStructCompletion(displayRequest:String, displaySource:String):String {
		...
		for (tokenIndex in 0...tokens.length) {
			final token = tokens[tokenIndex];
			if (token.start >= query.cursorOffset)
				break;
			switch (token.text) {
				case "(", "{", "[":
		...
		if (braceTokenIndex < 0 || parenTokenIndex < 0)
			return "";
	...
	public static function synthesize(displayRequest:String, displaySource:String):String {
		...
		if (exprOfCompletion.length > 0)
			return exprOfCompletion;

		final req = displayRequest == null ? "" : displayRequest;
		if (StringTools.endsWith(req, "@diagnostics"))
			return "[{\"diagnostics\":[]}]";
		if (StringTools.endsWith(req, "@module-symbols"))
			return "[{\"symbols\":[]}]";
		if (StringTools.endsWith(req, "@signature"))
			return "{\"signatures\":[],\"activeSignature\":0,\"activeParameter\":0}";
		if (StringTools.endsWith(req, "@toplevel"))
			return "<il></il>";
		if (StringTools.endsWith(req, "@type"))
			return "<type>Dynamic</type>";
		if (StringTools.endsWith(req, "@position"))
			return "<list></list>";
		if (StringTools.endsWith(req, "@usage"))
			return "<list></list>";
		return "<list></list>";
	}
}
