...
class MacroHostClient {
	...
	**/
	public static function encodeContextGetTypePayload(name:String):String {
		...
		if (resolved == null)
			return "";
		final resolvedTypes:Array<MacroResolvedModuleScanner.RuntimeResolvedTypeSnapshot> = MacroResolvedModuleScanner.scanResolvedModuleTypes(name,
			resolved.path, resolved.className);
