# 1 "Hxhx.ml"
(* Generated by reflaxe.ocaml: package alias module *)
module Macro = Hxhx_macro
module Runtime = Hxhx_runtime
module BackendPluginLoader = Hxhx_BackendPluginLoader
module BackendPluginManifestResolver = Hxhx_BackendPluginManifestResolver
module BackendProviderResolver = Hxhx_BackendProviderResolver
module BuildMetadataCollector = Hxhx_BuildMetadataCollector
module CliRouting = Hxhx_CliRouting
module CompilationRequestContext = Hxhx_CompilationRequestContext
module CompilationRequestOutput = Hxhx_CompilationRequestOutput
module CompilationRequestOutputEvent = Hxhx_CompilationRequestOutputEvent
module CompilationRequestOutputPaths = Hxhx_CompilationRequestOutputPaths
module CompilationRequestOutputTransaction = Hxhx_CompilationRequestOutputTransaction
module CompilationServerCachedParse = Hxhx_CompilationServerCachedParse
module CompilationServerCachedResolution = Hxhx_CompilationServerCachedResolution
module CompilationServerCachedSource = Hxhx_CompilationServerCachedSource
module CompilationServerDependencyCatalog = Hxhx_CompilationServerDependencyCatalog
module CompilationServerDependencyReport = Hxhx_CompilationServerDependencyReport
module CompilationServerDependencyRequest = Hxhx_CompilationServerDependencyRequest
module CompilationServerProtocol = Hxhx_CompilationServerProtocol
module CompilationServerReply = Hxhx_CompilationServerReply
module CompilationServerRequest = Hxhx_CompilationServerRequest
module CompilationServerRequestCodec = Hxhx_CompilationServerRequestCodec
module CompilationServerRequestDispatcher = Hxhx_CompilationServerRequestDispatcher
module CompilationServerSourceCache = Hxhx_CompilationServerSourceCache
module CompilationServerSourceCacheRequest = Hxhx_CompilationServerSourceCacheRequest
module CompilationServerStopSignal = Hxhx_CompilationServerStopSignal
module CompilerJsonArray = Hxhx_CompilerJsonArray
module CompilerJsonParser = Hxhx_CompilerJsonParser
module DisplayResponseSynthesizer = Hxhx_DisplayResponseSynthesizer
module ExprMacroExpander = Hxhx_ExprMacroExpander
module Hxml = Hxhx_Hxml
module LibraryResolver = Hxhx_LibraryResolver
module Main = Hxhx_Main
module NativeBackendPluginDynlink = Hxhx_NativeBackendPluginDynlink
module NativeBackendPluginHost = Hxhx_NativeBackendPluginHost
module NativeBackendPluginHostAbi = Hxhx_NativeBackendPluginHostAbi
module NativeBackendPluginLoader = Hxhx_NativeBackendPluginLoader
module Stage1Compiler = Hxhx_Stage1Compiler
module Stage3Args = Hxhx_Stage3Args
module Stage3BackendPluginSupport = Hxhx_Stage3BackendPluginSupport
module Stage3BuildMacroPreparationError = Hxhx_Stage3BuildMacroPreparationError
module Stage3BuildMacroPreparer = Hxhx_Stage3BuildMacroPreparer
module Stage3BuildMacroSupport = Hxhx_Stage3BuildMacroSupport
module Stage3Compiler = Hxhx_Stage3Compiler
module Stage3CustomizationSupport = Hxhx_Stage3CustomizationSupport
module Stage3DiagnosticsSupport = Hxhx_Stage3DiagnosticsSupport
module Stage3EmitSupport = Hxhx_Stage3EmitSupport
module Stage3HookSupport = Hxhx_Stage3HookSupport
module Stage3MacroHostSupport = Hxhx_Stage3MacroHostSupport
module Stage3PathSupport = Hxhx_Stage3PathSupport
module Stage3RunSupport = Hxhx_Stage3RunSupport
module Stage3SetupSupport = Hxhx_Stage3SetupSupport
module Stage3WaitServer = Hxhx_Stage3WaitServer
