...
let renderPrelude = fun out classes symbolTable -> ignore ((
  ...
  let _g = ref 0 in (
    ...
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
    ignore (HxArray.push out "var __hxhx_sys_system_name = function() {");
    ignore (HxArray.push out "  return $loader.loadprim(\"std@sys_string\", 0)();");
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
    ignore (HxArray.push out "var __hxhx_sys_get_env = function(name) {");
    ignore (HxArray.push out "  return $loader.loadprim(\"std@get_env\", 1)(name);");
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
    ignore (HxArray.push out "var __hxhx_sys_put_env = function(name, value) {");
    ignore (HxArray.push out "  return $loader.loadprim(\"std@put_env\", 2)(name, value);");
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
    ignore (HxArray.push out "var __hxhx_file_contents = $loader.loadprim(\"std@file_contents\", 1);");
    ignore (HxArray.push out "var __hxhx_json_min_field_from_file = function(path) {");
    ignore (HxArray.push out "  var text = __hxhx_file_contents(path);");
    ignore (HxArray.push out "  var marker = \"\\\"min\\\"\";");
    ignore (HxArray.push out "  var pos = try $sfind(text, 0, marker) catch e null;");
    ignore (HxArray.push out "  if (pos == null) return null;");
    ignore (HxArray.push out "  pos = pos + $ssize(marker);");
    ignore (HxArray.push out "  var size = $ssize(text);");
    ignore (HxArray.push out "  while (pos < size && $sget(text, pos) != 58) pos = pos + 1;");
    ignore (HxArray.push out "  if (pos >= size) return null;");
    ignore (HxArray.push out "  pos = pos + 1;");
    ignore (HxArray.push out "  var scanningWhitespace = true;");
    ignore (HxArray.push out "  while (scanningWhitespace && pos < size) {");
    ignore (HxArray.push out "    var c = $sget(text, pos);");
    ignore (HxArray.push out "    if (c == 32 || c == 9 || c == 10 || c == 13) pos = pos + 1; else scanningWhitespace = false;");
    ignore (HxArray.push out "  }");
    ignore (HxArray.push out "  var start = pos;");
    ignore (HxArray.push out "  if (pos < size && $sget(text, pos) == 45) pos = pos + 1;");
    ignore (HxArray.push out "  var scanningDigits = true;");
    ignore (HxArray.push out "  while (scanningDigits && pos < size) {");
    ignore (HxArray.push out "    var digit = $sget(text, pos);");
    ignore (HxArray.push out "    if (digit >= 48 && digit <= 57) pos = pos + 1; else scanningDigits = false;");
    ignore (HxArray.push out "  }");
    ignore (HxArray.push out "  if (pos == start) return null;");
    ignore (HxArray.push out "  if (pos == start + 1 && $sget(text, start) == 45) return null;");
    ignore (HxArray.push out "  return $int($ssub(text, start, pos - start));");
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
    ...
    ignore (HxArray.push out "    var buf = $smake(1);");
    ignore (HxArray.push out "    while (true) {");
    ignore (HxArray.push out "      var n = read(handle, buf, 0, 1);");
    ignore (HxArray.push out "      if (n <= 0) { if (out == \"\") $throw(\"Eof\"); return out; }");
    ignore (HxArray.push out "      var ch = $ssub(buf, 0, n);");
    ignore (HxArray.push out "      if (ch == \"\\n\") return out;");
    ignore (HxArray.push out "      if (ch != \"\\r\") out = out + ch;");
    ignore (HxArray.push out "    }");
    ignore (HxArray.push out "  };");
    ignore (HxArray.push out "  input.readAll = function() {");
    ignore (HxArray.push out "    var out = \"\";");
    ignore (HxArray.push out "    var buf = $smake(4096);");
    ignore (HxArray.push out "    while (true) {");
    ignore (HxArray.push out "      var n = try read(handle, buf, 0, 4096) catch e 0;");
    ignore (HxArray.push out "      if (n <= 0) return out;");
    ignore (HxArray.push out "      out = out + $ssub(buf, 0, n);");
    ignore (HxArray.push out "    }");
    ignore (HxArray.push out "  };");
    ignore (HxArray.push out "  return input;");
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
    ignore (HxArray.push out "var __hxhx_process_new = function(command, args, detached) {");
    ignore (HxArray.push out "  if (detached == true) $throw(\"Detached process unsupported on this platform\");");
    ignore (HxArray.push out "  if (args == null) args = $array();");
    ignore (HxArray.push out "  var run = $loader.loadprim(\"std@process_run\", 2);");
    ignore (HxArray.push out "  var stdoutRead = $loader.loadprim(\"std@process_stdout_read\", 4);");
    ignore (HxArray.push out "  var stderrRead = $loader.loadprim(\"std@process_stderr_read\", 4);");
    ignore (HxArray.push out "  var exit = $loader.loadprim(\"std@process_exit\", 1);");
    ignore (HxArray.push out "  var pid = $loader.loadprim(\"std@process_pid\", 1);");
    ignore (HxArray.push out "  var close = $loader.loadprim(\"std@process_close\", 1);");
    ignore (HxArray.push out "  var kill = $loader.loadprim(\"std@process_kill\", 1);");
    ignore (HxArray.push out "  var handle = run(command, args);");
    ignore (HxArray.push out "  var process = $new(null);");
    ignore (HxArray.push out "  process.__hx_ctor = \"sys.io.Process\";");
    ignore (HxArray.push out "  process.__hx_params = $array(command, args, detached);");
    ignore (HxArray.push out "  process.__hx_value = command;");
    ignore (HxArray.push out "  process.stdout = __hxhx_process_input(handle, stdoutRead);");
    ignore (HxArray.push out "  process.stderr = __hxhx_process_input(handle, stderrRead);");
    ignore (HxArray.push out "  process.stdin = null;");
    ignore (HxArray.push out "  process.getPid = function() { return pid(handle); };");
    ignore (HxArray.push out "  process.exitCode = $varargs(function(exitArgs) { if (exitArgs[0] == false) $throw(\"Non blocking exitCode() not supported on this platform\"); return exit(handle); });");
    ignore (HxArray.push out "  process.close = function() { close(handle); return null; };");
    ignore (HxArray.push out "  process.kill = function() { kill(handle); return null; };");
    ignore (HxArray.push out "  return process;");
    ignore (HxArray.push out "}");
    ignore (HxArray.push out "");
