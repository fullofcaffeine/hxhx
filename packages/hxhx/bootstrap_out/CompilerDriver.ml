...
let run = fun () -> let classPaths = let __arr_1 = HxArray.create () in (
  ignore (HxArray.push __arr_1 "examples/hih-compiler/fixtures/src");
  __arr_1
  ...
  let root = HxArray.get resolved 0 in let ast = ResolvedModule.getParsed root in let decl = ParsedModule.getDecl ast () in let pkg = HxModuleDecl.getPackagePath decl in let imports = HxModuleDecl.getImports decl in let mainClass = HxModuleDecl.getMainClass decl in (
    ...
    let tempString = ref "" in (
      ignore (if HxString.length pkg = 0 then let __assign_2 = "<none>" in (
        tempString := __assign_2;
        __assign_2
      ) else let __assign_3 = pkg in (
        tempString := __assign_3;
        __assign_3
      ));
      ignore (print_endline ("package=" ^ HxString.toStdString (!tempString)));
      ...
      let tempString1 = ref "" in (
        ignore (if HxClassDecl.getHasStaticMain mainClass then let __assign_4 = "yes" in (
          tempString1 := __assign_4;
          __assign_4
        ) else let __assign_5 = "no" in (
          tempString1 := __assign_5;
          __assign_5
        ));
        ignore (print_endline ("hasStaticMain=" ^ HxString.toStdString (!tempString1)));
        let fns = HxClassDecl.getFunctions mainClass in (
          ...
          ignore (let _g = ref 0 in while !_g < HxArray.length fns do ignore (let fn = HxArray.get fns (!_g) in (
            ...
            let tempString2 = ref "" in (
              ignore (let _g2 = HxFunctionDecl.getVisibility fn in match _g2 with
                | HxVisibility.Public -> let __assign_8 = "public" in (
                  tempString2 := __assign_8;
                  __assign_8
                )
                | HxVisibility.Private -> let __assign_9 = "private" in (
                  tempString2 := __assign_9;
                  __assign_9
                ));
              let vis = !tempString2 in let retHint = HxFunctionDecl.getReturnTypeHint fn in let tempString3 = ref "" in (
                ignore (if HxString.length retHint = 0 then let __assign_10 = "<none>" in (
                  tempString3 := __assign_10;
                  __assign_10
                ) else let __assign_11 = retHint in (
                  tempString3 := __assign_11;
                  __assign_11
                ));
                let ret = !tempString3 in let retStrLit = HxFunctionDecl.getReturnStringLiteral fn in let tempString4 = ref "" in (
                  ignore (if HxString.length retStrLit = 0 then let __assign_12 = "<none>" in (
                    tempString4 := __assign_12;
                    __assign_12
                  ) else let __assign_13 = retStrLit in (
                    tempString4 := __assign_13;
                    __assign_13
                  ));
                  let retStr = !tempString4 in let args = HxFunctionDecl.getArgs fn in let tempString5 = ref "" in (
                    ignore (if HxFunctionDecl.getIsStatic fn then let __assign_14 = "yes" in (
                      tempString5 := __assign_14;
                      __assign_14
                    ) else let __assign_15 = "no" in (
                      tempString5 := __assign_15;
                      __assign_15
                    ));
                    print_endline ((((((((((("fn=" ^ HxString.toStdString (HxFunctionDecl.getName fn)) ^ " static=") ^ HxString.toStdString (!tempString5)) ^ " vis=") ^ HxString.toStdString vis) ^ " args=") ^ string_of_int (HxArray.length args)) ^ " ret=") ^ HxString.toStdString ret) ^ " retStr=") ^ HxString.toStdString retStr)
                  )
                )
              )
            )
          ...
          let upstreamShaped = let __arr_20 = HxArray.create () in (
            ignore (HxArray.push __arr_20 (FrontendFixture.create "tests/misc/resolution/projects/spec/pack/Mod.hx" (HxArray.join (let __arr_21 = HxArray.create () in (
              ignore (HxArray.push __arr_21 "package pack;");
              ignore (HxArray.push __arr_21 "@:build(Macro.build()) class Mod {}");
              ignore (HxArray.push __arr_21 "@:build(Macro.build()) class ModSubType {}");
              __arr_21
            )) "\n" (fun x -> x)) "pack" "Mod" false));
            ignore (HxArray.push __arr_20 (FrontendFixture.create "tests/misc/resolution/projects/spec/pack/ModWithStatic.hx" (HxArray.join (let __arr_22 = HxArray.create () in (
              ignore (HxArray.push __arr_22 "package pack;");
              ignore (HxArray.push __arr_22 "");
              ignore (HxArray.push __arr_22 "class ModWithStatic {");
              ignore (HxArray.push __arr_22 "  public static function TheStatic() return \"pack.ModWithStatic.TheStatic function\";");
              ignore (HxArray.push __arr_22 "}");
              ignore (HxArray.push __arr_22 "");
              ignore (HxArray.push __arr_22 "@:build(Macro.build())");
              ignore (HxArray.push __arr_22 "class TheStatic {}");
              __arr_22
            ...
            ignore (let _g = ref 0 in while !_g < HxArray.length upstreamShaped do ignore (let case_ = HxArray.get upstreamShaped (!_g) in (
              ...
              let label = FrontendFixture.getLabel case_ () in let src = FrontendFixture.getSource case_ () in let parsed = ParsedModule.getDecl (ParserStage.parse src label) () in let parsedPkg = HxModuleDecl.getPackagePath parsed in let parsedMain = HxModuleDecl.getMainClass parsed in (
                ...
                ignore (if HxString.indexOf label "ModWithStatic" 0 >= 0 then ignore (let tempArray = ref (Obj.magic ()) in (
                  ignore (let _this = HxClassDecl.getFunctions parsedMain in let _g2 = let __arr_25 = HxArray.create () in __arr_25 in (
                ...
                let haxeDecl = HxParser.parseModule (HxParser.create src) (Obj.magic (HxRuntime.hx_null)) in (
                  ...
                  let haxeImports = HxModuleDecl.getImports haxeDecl in let parsedImports = HxModuleDecl.getImports parsed in (
                    ...
                    ignore (let _g2 = ref 0 in let _g1 = HxArray.length haxeImports in while !_g2 < _g1 do ignore (let i = let __old_29 = !_g2 in let __new_30 = HxInt.add __old_29 1 in (
                      ignore (_g2 := __new_30);
                      __old_29
                    ...
                    let haxeMain = HxModuleDecl.getMainClass haxeDecl in (
                      ...
                      let haxeFns = HxClassDecl.getFunctions haxeMain in let parsedFns = HxClassDecl.getFunctions parsedMain in (
                        ...
                        let _g2 = ref 0 in let _g1 = HxArray.length haxeFns in while !_g2 < _g1 do ignore (let i = let __old_31 = !_g2 in let __new_32 = HxInt.add __old_31 1 in (
                          ignore (_g2 := __new_32);
                          __old_31
            ...
            let typed = TyperStage.typeModule ast in (
              ...
              let typedFns = TyClassEnv.getFunctions (TyModuleEnv.getMainClass (TypedModule.getEnv typed ()) ()) () in (
                ...
                let expanded = MacroStage.expand typed (let __arr_35 = HxArray.create () in __arr_35) in (
