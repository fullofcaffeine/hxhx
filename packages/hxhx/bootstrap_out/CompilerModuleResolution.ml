...
let toOrigin = fun self (requestedModulePath : string) -> (
  ignore (if (Obj.magic self : t).filePath == Obj.magic (HxRuntime.hx_null) || (Obj.magic self : t).selectedClassPathIndex < 0 then HxType.hx_throw_typed_rtti (Obj.repr "missing module resolution has no source origin") ["Dynamic"] else ());
