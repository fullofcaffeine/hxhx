...
let render = fun expression scope services -> try let __fallback_result_11 = (
    ...
    let lookup = Obj.obj (HxAnon.get services "lookupForScope") scope in let owner = Obj.magic (Obj.obj (HxAnon.get services "ownerForType") (Obj.obj (HxAnon.get exact "owner") : string) scope lookup) in let representation = Obj.magic (Obj.obj (HxAnon.get services "representation") (Obj.magic owner) lookup) in (
      ...
      let helper = Obj.magic (Obj.obj (HxAnon.get services "method") (Obj.magic owner) (Obj.obj (HxAnon.get exact "method") : string)) in (
        ignore (if helper != Obj.magic (HxRuntime.hx_null) && Obj.obj (HxAnon.get services "isInline") (Obj.magic helper) then ignore (let tempMaybeString = ref (Obj.magic (HxRuntime.hx_null) : string) in (
          ...
          let inlineBody = (!tempMaybeString : string) in if inlineBody != Obj.magic (HxRuntime.hx_null) then raise (HxRuntime.Hx_return (Obj.repr (inlineBody : string))) else ()
        )) else ());
        let receiver = (Obj.obj (HxAnon.get services "valueForExpectedType") (Obj.magic (Obj.obj (HxEnum.unbox_or_obj "HxExpr" (HxAnon.get exact "receiver")))) (Backend_cpp_CppAbstractRepresentation.getCarrierCppType (Obj.magic representation) () : string) scope : string) in let _g = Obj.magic (let __arr_7 = HxArray.create () in __arr_7) in let _g1 = ref 0 in let _g2 = Obj.magic (Obj.obj (HxAnon.get exact "arguments")) in (
          ignore (while !_g1 < HxArray.length _g2 do ignore (let argument = Obj.magic (HxArray.get (Obj.magic _g2) (!_g1)) in (
            ...
            HxArray.push _g (Obj.obj (HxAnon.get services "renderExpression") (Obj.magic argument) scope)
          )) done);
