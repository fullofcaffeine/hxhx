...
class OcamlFunctionPlanSealer {
	...
	**/
	...
	public function seal(data:ClassFuncData):Void {
		...
		final reflectRuntimeUses = new OcamlReflectRuntimeUsePlanner(binding).plan(data.expr);
		for (decision in reflectRuntimeUses.decisions())
			context.recordReflectRuntimeUseRequirement(decision);
		...
		final typeOf = new OcamlTypeOfPlanner(binding).plan(data.expr);
		for (decision in typeOf.decisions())
			context.recordTypeOfRuntimeRequirement(decision);
		...
		final stringFromCharCode = new OcamlStringFromCharCodePlanner(binding).plan(data.expr);
		...
		function visit(expression:TypedExpr, lexicalParentBinding:OcamlFunctionPlanBinding):Void {
			switch (expression.expr) {
				case TFunction(tfunc):
					...
					for (decision in reflectRuntimeUses.decisions())
						context.recordReflectRuntimeUseRequirement(decision);
					final stdIsOfType = new OcamlStdIsOfTypePlanner(nestedBinding).plan(tfunc.expr);
					stdIsOfType.requirePlanBinding(nestedBinding);
					for (decision in stdIsOfType.decisions())
						context.recordStdIsOfTypeRuntimeRequirement(decision);
					final typeOf = new OcamlTypeOfPlanner(nestedBinding).plan(tfunc.expr);
					typeOf.requirePlanBinding(nestedBinding);
					for (decision in typeOf.decisions())
						context.recordTypeOfRuntimeRequirement(decision);
					...
					for (decision in intUnary.decisions())
						context.recordIntUnaryRuntimeRequirement(decision);
					final stringFromCharCode = new OcamlStringFromCharCodePlanner(nestedBinding).plan(tfunc.expr);
					stringFromCharCode.requirePlanBinding(nestedBinding);
					for (decision in stringFromCharCode.decisions())
						context.recordStringFromCharCodeRuntimeRequirement(decision);
					...
					for (decision in stringEquality.decisions())
						context.recordStringEqualityRuntimeRequirement(decision);
					final stringMethods = new OcamlStringMethodPlanner(nestedBinding).plan(tfunc.expr);
