...
class CppRuntimeSupport {
	...
	**/
	...
	/**
		Perform signed 64-bit carrier arithmetic and complement with Haxe semantics.
		...
		Complement uses the equivalent `-1 - value` identity, whose result is
		representable for every signed 64-bit input.
	**/
	public static function int64ArithmeticLines():Array<String> {
		return [
			...
			"}",
			"",
			"static long long __hxhx_int64_mul(long long left, long long right) {",
			"  static_assert(std::numeric_limits<unsigned long long>::digits == 64, \"hxhx Int64 requires a 64-bit carrier\");",
			"  const auto bits = static_cast<unsigned long long>(left) * static_cast<unsigned long long>(right);",
			"  const auto signedMax = static_cast<unsigned long long>(std::numeric_limits<long long>::max());",
			"  if (bits <= signedMax) return static_cast<long long>(bits);",
			...
			"}",
			"",
			"static long long __hxhx_int64_complement(long long value) {",
			"  static_assert(std::numeric_limits<unsigned long long>::digits == 64, \"hxhx Int64 requires a 64-bit carrier\");",
			"  return -1 - value;",
			"}"
		];
	...
	**/
	...
	public static function serializerEnumSupportLines(serializerName:String, unserializerName:String):Array<String> {
		return [
			"struct " + serializerName + " {",
			"  inline static bool USE_ENUM_INDEX = false;",
			"  std::string buf;",
			"  " + serializerName + "() = default;",
			"  template<typename T>",
			"  static std::string run(T value) {",
			"    auto serializer = std::make_shared<" + serializerName + ">();",
			"    serializer->serialize(value);",
			"    return serializer->toString();",
			"  }",
			"  std::string toString() const { return buf; }",
			"  void serializeString(const std::string& value) {",
			"    buf += std::string(\"y\") + std::to_string(value.size()) + std::string(\":\") + value;",
			"  }",
			"  void serialize(int value) { buf += std::string(\"i\") + std::to_string(value); }",
			"  void serialize(bool value) { buf += value ? std::string(\"t\") : std::string(\"f\"); }",
			"  void serialize(const std::string& value) { serializeString(value); }",
			"  void serialize(const char* value) { serializeString(value == nullptr ? std::string() : std::string(value)); }",
			"  void serialize(const std::shared_ptr<EnumValue>& value) {",
			"    if (value == nullptr) { buf += std::string(\"n\"); return; }",
			"    auto enumMeta = Type::resolveEnum(value->getEnumName());",
			"    auto enumName = enumMeta == nullptr ? value->getEnumName() : enumMeta->name;",
			"    if (" + serializerName + "::USE_ENUM_INDEX) {",
			"      buf += std::string(\"j\");",
			"      serializeString(enumName);",
			"      buf += std::string(\":\") + std::to_string(value->getIndex());",
			"    } else {",
			"      buf += std::string(\"w\");",
			"      serializeString(enumName);",
			"      serializeString(value->getName());",
			"    }",
			"    auto payloads = value->getPayloads();",
			"    if (payloads.empty() && !value->getParameters().empty()) for (const auto& param : value->getParameters()) payloads.push_back(std::any(param));",
			"    buf += std::string(\":\") + std::to_string(payloads.size());",
			"    for (const auto& payload : payloads) serialize(payload);",
			"  }",
			"  void serialize(const std::any& value) {",
			"    if (!value.has_value()) { buf += std::string(\"n\"); return; }",
			"    if (__hxhx_is_enum_value(value)) { serialize(__hxhx_enum_value_ptr(value)); return; }",
			"    if (value.type() == typeid(int)) { serialize(std::any_cast<int>(value)); return; }",
			"    if (value.type() == typeid(bool)) { serialize(std::any_cast<bool>(value)); return; }",
			"    if (value.type() == typeid(std::string)) { serialize(std::any_cast<std::string>(value)); return; }",
			"    if (value.type() == typeid(const char*)) { serialize(std::any_cast<const char*>(value)); return; }",
			"    buf += std::string(\"n\");",
			"  }",
			"  template<typename T>",
			"  void serialize(const std::shared_ptr<T>& value) {",
			"    if (value == nullptr) { buf += std::string(\"n\"); return; }",
			"    if constexpr (__hxhx_has_enum_metadata<T>::value) serialize(__hxhx_enum_to_erased(value));",
			"    else buf += std::string(\"n\");",
			"  }",
			"};",
			"",
			"struct " + unserializerName + " {",
			"  std::string buf;",
			"  int pos = 0;",
			"  int length = 0;",
			"  explicit " + unserializerName + "(std::string value = std::string()) : buf(value), pos(0), length(static_cast<int>(value.size())) {}",
			"  static std::any run(std::string value) {",
			"    auto unserializer = std::make_shared<" + unserializerName + ">(value);",
			"    return unserializer->unserialize();",
			"  }",
			"  int get(int index) const {",
			"    if (index < 0 || index >= length) throw std::runtime_error(std::string(\"Invalid object\"));",
			"    return static_cast<int>(static_cast<unsigned char>(buf[static_cast<std::size_t>(index)]));",
			"  }",
			"  int readDigits() {",
			"    int value = 0;",
			"    while (pos < length && buf[static_cast<std::size_t>(pos)] >= '0' && buf[static_cast<std::size_t>(pos)] <= '9') {",
			"      value = value * 10 + static_cast<int>(buf[static_cast<std::size_t>(pos)] - '0');",
			"      pos++;",
			"    }",
			"    return value;",
			"  }",
			...
			"    int len = readDigits();",
			"    if (get(pos++) != static_cast<int>(':')) throw std::runtime_error(std::string(\"Invalid string length\"));",
			"    if (len < 0 || pos + len > length) throw std::runtime_error(std::string(\"Invalid string length\"));",
			"    std::string out = buf.substr(static_cast<std::size_t>(pos), static_cast<std::size_t>(len));",
			"    pos += len;",
			"    return out;",
			"  }",
			"  std::any unserialize() {",
			"    int code = get(pos++);",
			"    switch (code) {",
			"      case 'n': return std::any();",
			"      case 't': return std::any(true);",
			"      case 'f': return std::any(false);",
			"      case 'i': {",
			"        int sign = 1;",
			"        if (pos < length && buf[static_cast<std::size_t>(pos)] == '-') { sign = -1; pos++; }",
			"        int value = readDigits();",
			"        return std::any(sign * value);",
			"      }",
			"      case 'y': return std::any(readString());",
			"      case 'w': {",
			"        auto enumName = std::any_cast<std::string>(unserialize());",
			"        auto tag = std::any_cast<std::string>(unserialize());",
			"        return std::any(unserializeEnum(Type::resolveEnum(enumName), tag));",
			"      }",
			"      case 'j': {",
			"        auto enumName = std::any_cast<std::string>(unserialize());",
			"        if (get(pos++) != static_cast<int>(':')) throw std::runtime_error(std::string(\"Invalid enum format\"));",
			"        int index = readDigits();",
			"        return std::any(unserializeEnumIndex(Type::resolveEnum(enumName), index));",
			"      }",
			"      default:",
			"        throw std::runtime_error(std::string(\"Invalid char \") + static_cast<char>(code));",
			"    }",
			"  }",
			"  std::shared_ptr<EnumValue> unserializeEnum(std::shared_ptr<Enum> edecl, std::string tag) {",
			"    if (get(pos++) != static_cast<int>(':')) throw std::runtime_error(std::string(\"Invalid enum format\"));",
			"    int nargs = readDigits();",
			"    if (nargs == 0) return Type::createEnum<std::shared_ptr<EnumValue>>(edecl, tag);",
			"    std::vector<std::any> args;",
			"    while (nargs-- > 0) args.push_back(unserialize());",
			"    return Type::createEnum<std::shared_ptr<EnumValue>>(edecl, tag, args);",
			"  }",
			"  std::shared_ptr<EnumValue> unserializeEnumIndex(std::shared_ptr<Enum> edecl, int index) {",
			"    if (get(pos++) != static_cast<int>(':')) throw std::runtime_error(std::string(\"Invalid enum format\"));",
			"    int nargs = readDigits();",
			"    if (nargs == 0) return Type::createEnumIndex<std::shared_ptr<EnumValue>>(edecl, index);",
			"    const __hxhx_enum_ctor_meta* ctor = edecl == nullptr ? nullptr : edecl->findByIndex(index);",
			"    std::string tag = ctor == nullptr ? std::string() : ctor->name;",
			"    std::vector<std::any> args;",
			"    while (nargs-- > 0) args.push_back(unserialize());",
			"    return Type::createEnum<std::shared_ptr<EnumValue>>(edecl, tag, args);",
			"  }",
	...
	public static function enumValueTypeLines():Array<String> {
		return [
			...
			"  std::string enumName;",
			"  explicit EnumValue(std::string tag = std::string(), int index = 0, std::vector<std::string> parameters = {}, std::vector<std::any> payloads = {}, std::string enumName = std::string()) : tag(tag), index(index), parameters(parameters), payloads(payloads), enumName(enumName) {}",
			...
			"  std::string getEnumName() const { return enumName; }",
			"};"
		];
	}
	...
	public static function eRegSupportLines(?includeMatchedPosStruct:Bool = false):Array<String> {
		...
		lines.push([
			...
			"  }",
			"",
			"  std::optional<std::string> matched(int n) {",
			"    if (!hasLastMatch) throw std::runtime_error(\"EReg.matched called without a successful match\");",
			"    if (n < 0 || n >= static_cast<int>(lastMatch.size())) throw std::runtime_error(\"EReg.matched capture index out of range\");",
			"    if (!lastMatch[n].matched) return std::nullopt;",
			...
			"    if (!hasLastMatch) throw std::runtime_error(\"EReg.matchedLeft called without a successful match\");",
			"    return lastInput.substr(0, static_cast<std::size_t>(absoluteMatchStart()));",
			"  }",
			"",
			"  std::string matchedRight() {",
			"    if (!hasLastMatch) throw std::runtime_error(\"EReg.matchedRight called without a successful match\");",
			...
			"    if (!hasLastMatch) throw std::runtime_error(\"EReg.matchedPos called without a successful match\");",
