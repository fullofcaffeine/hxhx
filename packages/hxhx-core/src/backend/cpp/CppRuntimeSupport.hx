...
class CppRuntimeSupport {
	...
	public static function sysEventLoopLines():Array<String> {
		return [
			...
			"}",
			"",
			"// hxhx-cpp-smoke-only: Timer.delay ignores callbacks and Timer.stop is a no-op; see docs/00-project/CPP_SYS_EVENT_LOOP_SMOKE_AUDIT.md.",
			"struct Timer {",
			"  static double stamp() {",
			...
			"};",
			"",
			"// hxhx-cpp-smoke-only: Http exposes callback/request shape only and performs no transport; see docs/00-project/CPP_SYS_EVENT_LOOP_SMOKE_AUDIT.md.",
			"struct Http {",
			...
			"};",
			"",
			"// hxhx-cpp-smoke-only: Lock/Mutex are non-blocking compile-shape shims; wait/tryAcquire always succeed.",
			"struct Lock {",
			"  void acquire() {}",
