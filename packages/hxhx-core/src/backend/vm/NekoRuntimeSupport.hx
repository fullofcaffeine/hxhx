...
class NekoRuntimeSupport {
	public static function renderPrelude(out:Array<String>, classes:Array<NekoRuntimeClassMeta>, ?symbolTable:String):Void {
		...
		out.push("}");
		out.push("");
		out.push("var __hxhx_sys_system_name = function() {");
		out.push("  return $loader.loadprim(\"std@sys_string\", 0)();");
		out.push("}");
		out.push("");
		out.push("var __hxhx_sys_get_env = function(name) {");
		...
		out.push("}");
		out.push("");
		out.push("var __hxhx_file_contents = $loader.loadprim(\"std@file_contents\", 1);");
		out.push("var __hxhx_json_min_field_from_file = function(path) {");
		out.push("  var text = __hxhx_file_contents(path);");
		out.push("  var marker = \"\\\"min\\\"\";");
		out.push("  var pos = try $sfind(text, 0, marker) catch e null;");
		out.push("  if (pos == null) return null;");
		out.push("  pos = pos + $ssize(marker);");
		out.push("  var size = $ssize(text);");
		out.push("  while (pos < size && $sget(text, pos) != 58) pos = pos + 1;");
		out.push("  if (pos >= size) return null;");
		out.push("  pos = pos + 1;");
		out.push("  var scanningWhitespace = true;");
		out.push("  while (scanningWhitespace && pos < size) {");
		out.push("    var c = $sget(text, pos);");
		out.push("    if (c == 32 || c == 9 || c == 10 || c == 13) pos = pos + 1; else scanningWhitespace = false;");
		out.push("  }");
		out.push("  var start = pos;");
		out.push("  if (pos < size && $sget(text, pos) == 45) pos = pos + 1;");
		out.push("  var scanningDigits = true;");
		out.push("  while (scanningDigits && pos < size) {");
		out.push("    var digit = $sget(text, pos);");
		out.push("    if (digit >= 48 && digit <= 57) pos = pos + 1; else scanningDigits = false;");
		out.push("  }");
		out.push("  if (pos == start) return null;");
		out.push("  if (pos == start + 1 && $sget(text, start) == 45) return null;");
		...
		out.push("    var out = \"\";");
		out.push("    var buf = $smake(1);");
		out.push("    while (true) {");
		out.push("      var n = read(handle, buf, 0, 1);");
		out.push("      if (n <= 0) { if (out == \"\") $throw(\"Eof\"); return out; }");
		out.push("      var ch = $ssub(buf, 0, n);");
		out.push("      if (ch == \"\\n\") return out;");
		out.push("      if (ch != \"\\r\") out = out + ch;");
		out.push("    }");
		out.push("  };");
		out.push("  input.readAll = function() {");
		out.push("    var out = \"\";");
		out.push("    var buf = $smake(4096);");
		out.push("    while (true) {");
		out.push("      var n = try read(handle, buf, 0, 4096) catch e 0;");
		out.push("      if (n <= 0) return out;");
		out.push("      out = out + $ssub(buf, 0, n);");
		out.push("    }");
		out.push("  };");
		out.push("  return input;");
		out.push("}");
		out.push("");
		out.push("var __hxhx_process_new = function(command, args, detached) {");
		out.push("  if (detached == true) $throw(\"Detached process unsupported on this platform\");");
		out.push("  if (args == null) args = $array();");
		out.push("  var run = $loader.loadprim(\"std@process_run\", 2);");
		out.push("  var stdoutRead = $loader.loadprim(\"std@process_stdout_read\", 4);");
		out.push("  var stderrRead = $loader.loadprim(\"std@process_stderr_read\", 4);");
		out.push("  var exit = $loader.loadprim(\"std@process_exit\", 1);");
		out.push("  var pid = $loader.loadprim(\"std@process_pid\", 1);");
		out.push("  var close = $loader.loadprim(\"std@process_close\", 1);");
		out.push("  var kill = $loader.loadprim(\"std@process_kill\", 1);");
		out.push("  var handle = run(command, args);");
		out.push("  var process = $new(null);");
		out.push("  process.__hx_ctor = \"sys.io.Process\";");
		out.push("  process.__hx_params = $array(command, args, detached);");
		out.push("  process.__hx_value = command;");
		out.push("  process.stdout = __hxhx_process_input(handle, stdoutRead);");
		out.push("  process.stderr = __hxhx_process_input(handle, stderrRead);");
		out.push("  process.stdin = null;");
		out.push("  process.getPid = function() { return pid(handle); };");
		out.push("  process.exitCode = $varargs(function(exitArgs) { if (exitArgs[0] == false) $throw(\"Non blocking exitCode() not supported on this platform\"); return exit(handle); });");
		out.push("  process.close = function() { close(handle); return null; };");
		out.push("  process.kill = function() { kill(handle); return null; };");
		out.push("  return process;");
		out.push("}");
		out.push("");
