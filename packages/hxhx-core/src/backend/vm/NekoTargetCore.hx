...
class NekoTargetCore {
	...
	static inline var SYMBOL_TABLE = "__hxhx_symbols";
	...
	static function renderProgram(program:GenIrProgram, context:BackendContext):String {
		...
		final typedProgram = new NekoTypedProgramProjection(program.getTypedProgramRevision().getCanonicalIdentity(), modules);
		final classMeta = buildRuntimeClassMeta(modules);
		validateRuntimeTypeOperands(typedProgram, modules);
		...
		out.push("var " + SYMBOL_TABLE + " = $new(null);");
		NekoRuntimeTypeRegistry.renderDefinition(out, typedProgram, modules, SYMBOL_TABLE);
		NekoRuntimeSupport.renderPrelude(out, classMeta, SYMBOL_TABLE, typedProgram);
		renderLoaderPathPrelude(out, context);
		// Neko closures capture bindings by value. A shared object lets earlier
		// bodies resolve later functions and constructors when the call occurs.
		...
		for (info in reachable.constructors)
			NekoConstructionEmitter.render(out, emitContext, info);
	...
	static function renderSplitProgram(program:GenIrProgram, context:BackendContext, entryPath:String):NekoSplitProgram {
		...
		final symbolSource = [
			"// Generated by hxhx native Neko backend symbol table",
			"var " + SYMBOL_TABLE + " = $new(null);"
		];
		NekoRuntimeTypeRegistry.renderDefinition(symbolSource, typedProgram, modules, SYMBOL_TABLE);
		symbolSource.push("$exports.symbols = " + SYMBOL_TABLE + ";");
		support.push({
			kind: "symbols",
			path: symbolsPath,
			source: symbolSource.join("\n") + "\n"
		});
		...
		while (helperIndex < emitContext.abstractHelpers.length) {
			...
			final out = splitChunkHeader(symbolsLoadName, classMeta, typedProgram);
		...
		while (constructorIndex < reachable.constructors.length) {
			...
			for (i in constructorIndex...end)
				NekoConstructionEmitter.render(out, emitContext, reachable.constructors[i]);
			support.push({kind: "chunk" + chunkIndex, path: chunkPath, source: out.join("\n")});
		...
		entry.push("var " + SYMBOL_TABLE + " = $loader.loadmodule(" + quote(symbolsLoadName) + ", $loader).symbols;");
	...
	static function splitChunkHeader(symbolsLoadName:String, classMeta:Array<NekoRuntimeClassMeta>, program:NekoTypedProgramProjection):Array<String> {
		final out = new Array<String>();
		out.push("// Generated by hxhx native Neko backend MVP support chunk");
		out.push("var " + SYMBOL_TABLE + " = $loader.loadmodule(" + quote(symbolsLoadName) + ", $loader).symbols;");
		out.push("");
		NekoRuntimeSupport.renderPrelude(out, classMeta, SYMBOL_TABLE, program);
		return out;
	}
	...
	**/
	...
	/** Unsupported categories fail before either layout publishes target files. */
	...
	static function constructionPlan(context:NekoEmitContext, info:NekoClassInfo):NekoClassConstructionPlan {
		return new NekoClassConstructionPlan(context.typedProgram.classGraph, context.typedProgram, context.typedProgram.requireClassIdentity(info.cls));
	}

	static function exactClassInfo(context:NekoEmitContext, owner:TypedBackendClassProjection):NekoClassInfo {
		for (info in context.classes)
			if (info.cls == owner.getDeclaration())
				return info;
	...
	static function collectReachable(context:NekoEmitContext, mainInfo:NekoClassInfo):NekoReachable {
		...
		for (info in context.classes) {
			...
			reserved.push(mangleInitializer(context.typedProgram.requireClassIdentity(info.cls)));
		...
		addConstructor = function(info:NekoClassInfo):Void {
			...
			final lineage = constructionPlan(context, info).lineage;
			if (lineage.length > 1)
				addConstructor(exactClassInfo(context, lineage[1]));
			for (field in HxClassDecl.getFields(info.cls)) {
				if (!HxFieldDecl.getIsStatic(field) && HxFieldDecl.getInit(field) != null)
					collectExprRefs(withFieldInitializer(withCurrentClass(context, info), field), HxFieldDecl.getInit(field), addConstructor, addStatic);
			}
			for (fn in HxClassDecl.getFunctions(info.cls)) {
				if (!HxFunctionDecl.getIsStatic(fn) && !isMacroFunction(fn)) {
					for (stmt in HxFunctionDecl.getBody(fn))
						collectStmtRefs(withFunctionArgs(withCurrentClass(context, info), fn), stmt, addConstructor, addStatic);
				}
			}
		};
		...
		addStatic = function(info:NekoClassInfo, fn:HxFunctionDecl):Void {
			...
			final staticContext = withFunctionArgs(withCurrentClass(context, info), fn);
			for (stmt in HxFunctionDecl.getBody(fn))
	...
	static function collectExprRefs(context:NekoEmitContext, expr:HxExpr, addConstructor:NekoClassInfo->Void,
		...
		if (runtimeType != null) {
			if (runtimeType.getValue() != null)
				collectExprRefs(context, runtimeType.getValue(), addConstructor, addStatic);
			return;
		}
		...
		if (exactCall != null) {
			if (exactCall.usesAbstractReceiver()) {
				...
				if (!context.abstractHelperIds.exists(identity)) {
					...
					final helperContext = withFunctionArgs(withCurrentClass(context, exactHelperClass(context, helper)), helper.body.getDeclaration());
	...
	**/
	...
	static function renderSpecialFunction(out:Array<String>, context:NekoEmitContext, info:NekoClassInfo, fn:HxFunctionDecl):Bool {
		...
		if (info.fullName == "Type") {
			switch (fnName) {
				case "getClass":
					renderRuntimeForwarder(out, context, info, fn, context.typedProgram.runtimeHelperName("__hxhx_type_get_class") + "(o)");
					return true;
				case "getClassName":
					renderRuntimeForwarder(out, context, info, fn, context.typedProgram.runtimeHelperName("__hxhx_type_class_name") + "(c)");
					return true;
	...
	static function renderStmt(out:Array<String>, context:NekoEmitContext, stmt:HxStmt, indent:String):Void {
		...
		switch (stmt) {
			...
			case SExpr(ECall(ESuper, args), _):
				if (context.currentClass == null || context.selfName == null)
					throw "Neko superclass construction requires an instance scope";
				final lineage = constructionPlan(context, context.currentClass).lineage;
				if (lineage.length < 2)
					throw "Neko superclass construction has no exact parent";
				final parent = exactClassInfo(context, lineage[1]);
				final arguments = [context.selfName].concat([for (arg in args) renderExpr(context, arg)]);
				out.push(indent + renderInitializerRef(context, parent) + "(" + arguments.join(", ") + ");");
	...
	static function renderCall(context:NekoEmitContext, callee:HxExpr, args:Array<HxExpr>, ?selectedStatic:NekoProjectedFunction):String {
		...
		final declaredMember = switch (callee) {
			case EIdent(name) if (context.currentClass != null && !isLocalName(context, name)): isCurrentStaticFunction(context,
					...
					name) != null || (context.selfName != null
					&& (isCurrentInstanceMethod(context, name) || isCurrentInstanceField(context, name)));
			case _: false;
		};
		final stackPrimitive = declaredMember ? null : NekoStackPrimitives.renderCall(callee, renderedArgs);
		if (stackPrimitive != null)
		...
		switch (callee) {
			...
			case EField(EIdent("Type"), "getClass") if (args.length >= 1):
				return context.typedProgram.runtimeHelperName("__hxhx_type_get_class") + "(" + renderedArgs[0] + ")";
			case EField(EIdent("Type"), "getClassName") if (args.length >= 1):
				return context.typedProgram.runtimeHelperName("__hxhx_type_class_name") + "(" + renderedArgs[0] + ")";
	...
	**/
	...
	static function isCurrentInstanceMethod(context:NekoEmitContext, name:String):Bool {
		if (context.currentClass == null)
			return false;
		return constructionPlan(context, context.currentClass).hasInstanceMethod(name);
	}
	...
	**/
	...
	static function isCurrentInstanceField(context:NekoEmitContext, name:String):Bool {
		if (context.currentClass == null)
			return false;
		return constructionPlan(context, context.currentClass).hasInstanceField(name);
	}
	...
	static function childContext(context:NekoEmitContext):NekoEmitContext {
		return {
			...
			packFunctionArguments: context.packFunctionArguments,
	...
	static function mangleInitializer(classIdentity:String):String {
		return "__hxhx_init_" + haxe.crypto.Sha256.encode(classIdentity);
	}

	static function renderInitializerRef(context:NekoEmitContext, info:NekoClassInfo):String {
		return context.symbolTable + "." + mangleInitializer(context.typedProgram.requireClassIdentity(info.cls));
	}
