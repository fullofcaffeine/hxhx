...
class SourceTargetCommon {
	...
	**/
	...
	static function appendLuaMainStaticHelpers(out:Array<String>, decl:HxModuleDecl, className:String):Void {
		final helpers = new Array<HxFunctionDecl>();
		for (fn in HxClassDecl.getFunctions(HxModuleDecl.getMainClass(decl))) {
			final fnName = HxFunctionDecl.getName(fn);
			if (fnName == "main"
				|| fnName == "new"
		...
		final emitted = new Map<String, Bool>();
		for (fn in helpers) {
			final methodName = valueName(Lua, HxFunctionDecl.getName(fn));
			final key = methodName + "#" + Std.string(HxFunctionDecl.getArgs(fn).length);
			if (emitted.exists(key))
				continue;
			emitted.set(key, true);
		...
		switch (target) {
			...
			case Lua:
				...
				for (line in renderLuaSupportPrelude(program, decl, className))
					lines.push(line);
				appendLuaMainStaticHelpers(lines, decl, className);
