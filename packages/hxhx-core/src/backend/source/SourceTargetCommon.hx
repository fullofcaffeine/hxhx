	...
	static function renderProgram(target:SourceNativeTarget, program:GenIrProgram, decl:HxModuleDecl, className:String, body:Array<HxStmt>):String {
		...
		switch (target) {
			...
			case Php:
				...
				lines.push("    }");
				lines.push("    private static function encodeValue($value) {");
				lines.push("      if ($value instanceof \\__HxArray) $value = $value->toArray();");
				lines.push("      if (is_array($value)) {");
				lines.push("        $out = [];");
				lines.push("        foreach ($value as $key => $item) $out[$key] = self::encodeValue($item);");
				lines.push("        return $out;");
				lines.push("      }");
				lines.push("      if (is_object($value)) {");
				lines.push("        if (property_exists($value, \"__hx_value\")) return self::encodeValue($value->__hx_value);");
				lines.push("        $out = new \\stdClass();");
				lines.push("        foreach (get_object_vars($value) as $key => $item) {");
				lines.push("          if (strpos($key, \"__hx_\") === 0) continue;");
				lines.push("          $out->$key = self::encodeValue($item);");
				lines.push("        }");
				lines.push("        return $out;");
				lines.push("      }");
				lines.push("      return $value;");
				...
				lines.push("      return json_encode(self::encodeValue($value), JSON_UNESCAPED_SLASHES);");
				lines.push("    }");
				lines.push("  }");
				lines.push("}");
