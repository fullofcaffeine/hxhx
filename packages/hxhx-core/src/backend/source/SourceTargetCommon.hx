	...
	static function renderProgram(target:SourceNativeTarget, program:GenIrProgram, context:BackendContext, decl:HxModuleDecl, className:String,
		...
		switch (target) {
			...
			case Php:
				...
				lines.push("    $vars = get_class_vars($runtime);");
				lines.push("    if (array_key_exists(\"__hx_enum_ctors\", $vars) && is_array($vars[\"__hx_enum_ctors\"])) return new __HxArray(array_values($vars[\"__hx_enum_ctors\"]));");
				lines.push("    $ctors = [];");
				lines.push("    foreach ($vars as $name => $value) {");
				...
				lines.push("    return new __HxArray(array_values($ctors));");
				lines.push("  }");
