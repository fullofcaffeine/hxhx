...
class EmitterStage {
	...
	**/
	...
	/**
		...
		The signature index stores qualified keys such as `Haxe_Timer.stamp`.
		Numeric classification must apply the current module's selected import before
		using a short name. Otherwise, an unrelated `Timer` module can hide the
		declaration selected by `import haxe.Timer`.
	**/
	static function qualifiedCallSigForStage3(owner:HxExpr, field:String, ?callSigByCallee:Map<String, EmitterCallSig>,
			?staticImportByIdent:Map<String, String>):Null<EmitterCallSig> {
		...
		var moduleName = ocamlModuleNameFromTypePathParts(parts);
		if (parts.length == 1) {
			final importedModule = staticImportModuleForStage3(parts[0], staticImportByIdent);
	...
	**/
	...
	/**
		Return the declared parameter types from an explicitly typed function value.

		Typed-body projection represents a compiler-created temporary as an immediately
		called function and attaches the temporary's Haxe type to that function. Native
		Stage3 may use those explicit types when its older optional map argument is not
		readable. An ordinary untyped function is deliberately excluded: guessing that
		all function parameters have one OCaml representation can make a Haxe `Dynamic`
		value incorrectly behave like both an OCaml `bool` and an OCaml `int`.
	**/
	static function stage3TypedLambdaArgumentTypes(typeHint:String, argumentCount:Int):Null<Array<TyType>> {
		final functionType = TyType.fromHintText(typeHint);
		if (!functionType.isFunction())
			return null;
		final arguments = functionType.getFunctionArguments();
		if (arguments.length != argumentCount)
			return null;
		for (argument in arguments)
			if (argument == null || argument.isUnknown() || argument.isDynamic())
				return null;
		return arguments;
	}

	/**
		Lower one explicitly typed nested function while its parameters are visible.

		Native Stage3 currently receives some optional map arguments through an erased
		OCaml boundary. Statement-local entries are the stable fallback already used by
		identifier lookup, so parameters whose concrete types came from the function's
		type ascription are added for the recursive lowering call. The previous scope
		is restored on success or failure so compiler-server requests cannot leak names.
	**/
	static function withStage3TemporaryTypeEntries(names:Array<String>, types:Array<TyType>, render:Void->String):String {
		...
		if (names != null)
			for (index in 0...names.length) {
				...
				final type = types != null && index < types.length ? types[index] : null;
				if (name != null && name.length > 0 && name != "_" && type != null && !containsName(name))
					next.push(new _LocalTyEntry(name, type.toString()));
			}
		if (previous != null)
	...
	**/
	static function stage3IsFloatExpr(expr:HxExpr, ?tyByIdent:Map<String, TyType>, ?callSigByCallee:Map<String, EmitterCallSig>,
		...
		return switch (expr) {
			...
			case ECall(EField(owner, name), _): var signature = qualifiedCallSigForStage3(owner, name, callSigByCallee,
					staticImportByIdent); if (signature == null) signature = callSigForStage3(name,
					callSigByCallee); if (signature == null) signature = callSigForStage3(ocamlValueIdent(name),
					callSigByCallee); signature != null && StringTools.trim(signature.resultTypeHint) == "Float";
			case EBinop("/", a, b): !(stage3IsInt64Expr(a,
					tyByIdent) || stage3IsInt64Expr(b,
						tyByIdent)) && (stage3IsFloatExpr(a, tyByIdent, callSigByCallee, staticImportByIdent)
					|| stage3IsFloatExpr(b, tyByIdent, callSigByCallee, staticImportByIdent)
					|| (stage3IsIntExpr(a, tyByIdent) && stage3IsIntExpr(b, tyByIdent)));
			case EBinop(op, a, b) if (op == "+" || op == "-" || op == "*"): stage3IsFloatExpr(a, tyByIdent, callSigByCallee,
					staticImportByIdent) || stage3IsFloatExpr(b, tyByIdent, callSigByCallee, staticImportByIdent);
			case ETernary(_cond, thenExpr, elseExpr): stage3IsFloatExpr(thenExpr, tyByIdent, callSigByCallee,
					staticImportByIdent) && stage3IsFloatExpr(elseExpr, tyByIdent, callSigByCallee, staticImportByIdent);
			case _:
				false;
		}
	}
		...
		return switch (expr) {
			...
			case _
				if (stage3IsIntExpr(expr, tyByIdent)
					|| stage3IsFloatExpr(expr, tyByIdent, callSigByCallee, staticImportByIdent)
	...
	static function tryExprToOcamlStage3NumericStringIntrinsic(e:HxExpr, ?arityByIdent:Map<String, Int>, ?tyByIdent:Map<String, TyType>,
		...
		switch (e) {
			case ECall(EField(EIdent("Math"), "abs"), [arg]):
				final absFn = stage3IsFloatExpr(arg, tyByIdent, callSigByCallee,
					staticImportByIdent) ? "abs_float " : (stage3IsIntExpr(arg, tyByIdent) ? "abs " : "abs_float ");
				return absFn
					+ "("
	...
	static function exprToOcaml(e:HxExpr, ?arityByIdent:Map<String, Int>, ?tyByIdent:Map<String, TyType>, ?staticImportByIdent:Map<String, String>,
		...
		return switch (e) {
			...
			case EVariableDeclaration(_, _, _, _, _, _):
				throw "stage3 emitter: a variable declaration must remain inside its expression declaration list";
				...
				if (op == HxUnaryOperator.Increment || op == HxUnaryOperator.Decrement) {
					...
					switch (expr) {
						...
						case _:
							...
							if (forceFloatUnop || stage3IsFloatExpr(expr, tyByIdentRaw, callSigByCalleeRaw, staticImportByIdentRaw)) {
								"(-.(" + renderedExpr + "))";
				...
				switch (op) {
					...
					case "/":
						if (stage3IsInt64Expr(a, tyByIdentRaw) || stage3IsInt64Expr(b, tyByIdentRaw)) {
							...
							final aIsF = stage3IsFloatExpr(a, tyByIdentRaw, callSigByCalleeRaw, staticImportByIdentRaw);
							final bIsF = stage3IsFloatExpr(b, tyByIdentRaw, callSigByCalleeRaw, staticImportByIdentRaw);
					...
					case "==":
						if (stage3IsFloatExpr(a, tyByIdentRaw, callSigByCalleeRaw, staticImportByIdentRaw)
							|| stage3IsFloatExpr(b, tyByIdentRaw, callSigByCalleeRaw, staticImportByIdentRaw)) {
	...
	**/
	...
	static function stage3BootstrapDefineValue(name:String):Null<String> {
		if (name == null || name.length == 0)
			return null;
		final args = Sys.args();
		final prefix = name + "=";
	...
	static inline function stage3BootstrapWantsPluginDuneLayout():Bool {
		return stage3BootstrapDefineValue("ocaml_dune_layout") == "plugin";
	}

	static function escapeOcamlStringLiteralForStage3(value:String):String {
		final raw = value == null ? "" : value;
		final out = new StringBuf();
		for (idx in 0...raw.length) {
			final code = raw.charCodeAt(idx);
			switch (code) {
				case 34:
					out.add("\\\"");
				case 92:
	...
	**/
	...
	public static function emitToDir(p:MacroExpandedProgram, outDir:String, emitFullBodies:Bool = false, buildExecutable:Bool = true,
		...
		**/
		...
		final exePath = EmitterStageBuildSupport.expectedExePath(outAbs);
