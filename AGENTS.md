...
- Before running a commit or any other hook-capable command in a sibling worktree, inspect the repository's installed hooks and Beads/tooling path resolution. Linked worktrees can still share hooks, databases, and metadata paths with the primary checkout. Record the primary checkout's HEAD plus staged, unstaged, and untracked-state digests before the command and compare them afterward.
- If a sibling hook cannot be proven worktree-local, run its relevant safety checks explicitly inside the task worktree and bypass that hook only for the isolated task commit. Record the bypass and equivalent checks in the commit or PR evidence. Never let a sibling-worktree hook export, format, stage, or otherwise rewrite the primary checkout. If the primary fingerprint changes, stop, preserve both states, report the exact paths, and do not guess at restoring another agent's work.
