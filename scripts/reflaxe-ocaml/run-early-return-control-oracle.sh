...
FIXTURE_DIR="$ROOT/test/oracle/reflaxe_ocaml_early_return_control_seed"
SRC_DIR="$FIXTURE_DIR/src"
EXPECTED="$FIXTURE_DIR/expected.stdout"
OUT_DIR="${REFLAXE_OCAML_EARLY_RETURN_ORACLE_OUT_DIR:-$ROOT/.tmp/reflaxe-ocaml-early-return-control-oracle}"

if ! command -v "$HAXE_BIN" >/dev/null 2>&1; then
	echo "reflaxe.ocaml early-return control oracle: missing Haxe compiler '$HAXE_BIN'" >&2
	exit 127
fi
if ! command -v node >/dev/null 2>&1; then
	echo "reflaxe.ocaml early-return control oracle: missing Node.js" >&2
	exit 127
fi
if ! command -v neko >/dev/null 2>&1; then
	echo "reflaxe.ocaml early-return control oracle: missing Neko" >&2
	exit 127
fi

HAXE_VERSION="$("$HAXE_BIN" --version)"
if [[ "$HAXE_VERSION" != "4.3.7" ]]; then
	echo "reflaxe.ocaml early-return control oracle: expected upstream Haxe 4.3.7, got '$HAXE_VERSION'" >&2
	exit 2
fi
...
echo "REFLAXE_OCAML_EARLY_RETURN_CONTROL_ORACLE:PASS routes=3 shared_cases=36 target_specific_cases=1"
