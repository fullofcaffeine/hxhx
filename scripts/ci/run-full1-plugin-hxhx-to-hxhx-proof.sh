...
PLUGIN_ID="full1.reflaxe.ocaml.hxhx"
PROVIDER_TYPE="backend.js.JsBackend"
TARGET_ID="js-native"
RUN_ID="$(date +%Y%m%d-%H%M%S)"
ARTIFACT_DIR="${FULL1_PLUGIN_HXHX_ARTIFACT_DIR:-$ROOT/.artifacts/full1/plugin-hxhx-to-hxhx/$RUN_ID}"
KEEP_TMP="${FULL1_PLUGIN_HXHX_KEEP_TMP:-0}"
...
require_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "full1 plugin hxhx-to-hxhx proof: missing required command: $1" >&2
    exit 1
  fi
}
...
read_abi_const() {
  ...
  if [ -z "$value" ]; then
    echo "full1 plugin hxhx-to-hxhx proof: failed to read $name from $ABI_FILE" >&2
    exit 1
  fi
  printf '%s' "$value"
}
...
resolve_hxhx_bin() {
  local candidate="${FULL1_PLUGIN_HXHX_BIN:-${HXHX_BIN:-}}"
  ...
  if [ ! -x "$BUILD_HXHX_SCRIPT" ]; then
    echo "full1 plugin hxhx-to-hxhx proof: missing hxhx build script: $BUILD_HXHX_SCRIPT" >&2
    exit 1
  fi

  HXHX_FORCE_STAGE0="${FULL1_PLUGIN_HXHX_FORCE_STAGE0_BUILD:-1}" \
  HXHX_FORBID_STAGE0=0 \
  HXHX_STAGE0_OCAML_BUILD="${FULL1_PLUGIN_HXHX_STAGE0_OCAML_BUILD:-byte}" \
  HXHX_STAGE0_FAILFAST_SECS="${FULL1_PLUGIN_HXHX_BUILD_FAILFAST_SECS:-1800}" \
...
if [ ! -f "$ABI_FILE" ]; then
  echo "full1 plugin hxhx-to-hxhx proof: missing ABI file: $ABI_FILE" >&2
  exit 1
fi
...
cleanup() {
  if [ "$KEEP_TMP" = "1" ]; then
    echo "full1_plugin_hxhx_tmp=$tmp_root"
  else
    rm -rf "$tmp_root"
  fi
}
...
if [ -z "$hxhx_bin" ] || [ ! -x "$hxhx_bin" ]; then
  echo "full1 plugin hxhx-to-hxhx proof: failed to resolve executable hxhx binary: $hxhx_bin" >&2
  exit 1
fi
...
compile_stdout="$ARTIFACT_DIR/hxhx-plugin-build.stdout.log"
compile_stderr="$ARTIFACT_DIR/hxhx-plugin-build.stderr.log"
...
summary_json="$ARTIFACT_DIR/full1-plugin-hxhx-to-hxhx.summary.json"
...
if [ -z "$dune_exe_name" ]; then
  echo "full1 plugin hxhx-to-hxhx proof: failed to read generated dune executable name" >&2
  exit 1
fi
...
if [ ! -f "$generated_artifact" ]; then
  echo "full1 plugin hxhx-to-hxhx proof: missing generated plugin artifact: $generated_artifact" >&2
  exit 1
fi
...
if [ "$hxhx_status" -ne 0 ]; then
  echo "full1 plugin hxhx-to-hxhx proof: hxhx compile failed (exit $hxhx_status)" >&2
  cat "$hxhx_stdout" >&2
  cat "$hxhx_stderr" >&2
  exit "$hxhx_status"
fi

if ! grep -q '^full1_hxhx_plugin_loaded=ok$' "$hxhx_stdout"; then
	echo "full1 plugin hxhx-to-hxhx proof: missing plugin load side-effect marker" >&2
	cat "$hxhx_stdout" >&2
	exit 1
fi
if ! grep -Eq '^backend_plugin_manifest\[[^]]+\]\[[^]]+\]=1$' "$hxhx_stdout"; then
	echo "full1 plugin hxhx-to-hxhx proof: manifest did not register exactly one provider" >&2
	cat "$hxhx_stdout" >&2
	exit 1
fi
if ! grep -q '^backend_selected_impl=provider/js-native-wrapper$' "$hxhx_stdout"; then
	echo "full1 plugin hxhx-to-hxhx proof: missing backend selection marker" >&2
	cat "$hxhx_stdout" >&2
  exit 1
fi
if [ ! -f "$compile_out_dir/main.js" ]; then
  echo "full1 plugin hxhx-to-hxhx proof: missing generated JS output" >&2
  exit 1
fi

node "$compile_out_dir/main.js" >"$node_stdout"
if ! grep -q '^sum=6$' "$node_stdout"; then
  echo "full1 plugin hxhx-to-hxhx proof: unexpected generated program output" >&2
  cat "$node_stdout" >&2
  exit 1
fi
...
cat >"$summary_json" <<EOF
{
  ...
  "runId": "$RUN_ID",
  "proof": "reflaxe.ocaml hxhx-built plugin artifact loaded by hxhx",
  "repoRoot": "$ROOT_JSON",
  "tmpRoot": "$TMP_JSON",
  "pluginCompiler": {
    "kind": "hxhx-stage3",
  ...
  "sourceBuildCompiler": {
    "kind": "stage0-haxe-for-hxhx-binary-build",
    "path": "$HAXE_BIN_JSON",
  ...
  "plugin": {
    ...
    "loadSideEffect": "full1_hxhx_plugin_loaded=ok"
  },
  "sampleCompile": {
  ...
  "logs": {
    "hxhxPluginBuildStdout": "hxhx-plugin-build.stdout.log",
    "hxhxPluginBuildStderr": "hxhx-plugin-build.stderr.log",
  ...
  "result": "REFLAXE_OCAML_PLUGIN_HXHX_TO_HXHX:PASS"
}
EOF

echo "full1_plugin_hxhx_to_hxhx_summary=$summary_json"
echo "full1_plugin_hxhx_to_hxhx_artifact_sha256=$plugin_artifact_sha256"
echo "REFLAXE_OCAML_PLUGIN_HXHX_TO_HXHX:PASS"
