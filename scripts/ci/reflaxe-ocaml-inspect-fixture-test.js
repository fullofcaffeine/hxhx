...
try {
	...
	assert.strictEqual(report.schemaVersion, 46)
	...
	assert.strictEqual(report.lowering.stdIsOfType.length, report.summary.stdIsOfTypeCount)
	assert(report.lowering.stdIsOfType.length > 0)
	assert.match(report.lowering.stdIsOfTypeRevision, sha256Revision)
	...
	assert(human.stdout.includes('[PASS] Std.isOfType:'))
	...
	const corruptTypeCheckReport = JSON.parse(corruptTypeCheckResult.stdout)
	assert.strictEqual(corruptTypeCheckReport.lowering.status, 'invalid')
	assert(corruptTypeCheckReport.lowering.message.includes('stale strategy or runtime-use facts'))
