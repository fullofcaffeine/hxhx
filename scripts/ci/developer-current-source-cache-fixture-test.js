...
try {
  ...
  write(fakeBuildDriver, '#!/usr/bin/env bash\nprintf \'%s\\n\' "$HXHX_FIXTURE_BIN"\n', 0o755)
  write(
    fixtureConfig,
    `${JSON.stringify(
      {
        ...
        buildEnvironmentNames: ['HXHX_STAGE0_NO_OPT'],
      },
      null,
      2
    )}\n`
  ...
  function fingerprint() {
    const result = run(process.execPath, [fingerprintTool, '--root', repo, '--json-out', report], {
      env: fixtureEnv,
    })
