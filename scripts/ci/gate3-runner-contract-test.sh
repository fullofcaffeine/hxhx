...
grep -Fq 'resolve_system_nekoc_bin()' "$runner"
grep -Fq 'system_neko="$(resolve_system_neko_bin || true)"' "$runner"
grep -Fq 'system_nekoc="$(resolve_system_nekoc_bin || true)"' "$runner"
grep -Fq 'STAGE0_NEKO="$system_neko"' "$runner"
grep -Fq 'STAGE0_NEKOC="$system_nekoc"' "$runner"
...
grep -Fq 'if [ -n "\${NEKOPATH:-}" ]; then' "$runner"
grep -Fq 'export NEKOPATH="\${NEKOPATH}:${NEKOPATH_DIR}"' "$runner"
grep -Fq 'unset NEKOPATH' "$runner"
grep -Fq 'unset LD_LIBRARY_PATH' "$runner"
grep -Fq 'cat >"$WRAP_DIR/nekoc"' "$runner"
grep -Fq 'exec "${STAGE0_NEKOC}" "\$@"' "$runner"
...
grep -Fq 'gate3_target_attempt_start target=${target}' "$runner"
grep -Fq 'gate3_target_attempt_end target=${target}' "$runner"
grep -Fq 'target_timeout_for()' "$runner"
grep -Fq 'HXHX_GATE3_TARGET_TIMEOUT_${target_key}_SEC' "$runner"
grep -Fq 'capture_target_failure_artifacts "$target" "$t_lower" "$code"' "$runner"
grep -Fq "find \"\$UPSTREAM_DIR/tests\" \\( -path '*/bin/*.neko' -o -path '*/export/*.neko' \\) -type f -size -2M" "$runner"
grep -Fq 'haxelib_path_dummy_ndll.txt' "$runner"
grep -Fq 'PATH="$WRAP_DIR:$PATH" haxelib --always path dummy_ndll' "$runner"
grep -Fq '.artifacts/full1/gate3-target-artifacts/**' "$extended_workflow"
grep -Fq 'gate2_stage3_emit_runner_start pid=${cmd_pid}' "$macro_runner"
grep -Fq 'gate2_stage3_emit_runner_end status=${status_label}' "$macro_runner"
grep -Fq 'if [ -n "\${NEKOPATH:-}" ]; then' "$macro_runner"
grep -Fq 'export NEKOPATH="\${NEKOPATH}:${NEKOPATH_DIR}"' "$macro_runner"
grep -Fq 'gate3_target_attempt_end status=<pass|fail|timeout>' "$testing_doc"
grep -Fq 'validate-current-source-hxhx-bin.sh' "$runner"
grep -Fq 'validate-current-source-hxhx-bin.sh' "$macro_runner"
grep -Fq 'HXHX_REQUIRE_CURRENT_SOURCE_BIN=1' "$testing_doc"
grep -Fq 'HXHX_GATE3_USE_PROVIDED_UPSTREAM_WORKTREE' "$runner"
grep -Fq 'Provided upstream worktree must be checked out at ${UPSTREAM_REF}' "$runner"
grep -Fq 'Provided HXHX_BIN does not exist' "$runner"
grep -Fq 'HXHX_BIN="$ROOT/$HXHX_BIN"' "$runner"
grep -Fq 'node scripts/ci/run-gate3-targets-parallel.js' "$extended_workflow"
grep -Fq 'FULL1_GATE3_EXTENDED_TARGET_JOBS: "3"' "$extended_workflow"
grep -Fq '.artifacts/full1/gate3-target-logs/**' "$extended_workflow"
grep -Fq '.artifacts/full1/gate3-target-summaries/**' "$extended_workflow"
...
grep -Fq 'HXHX_GATE3_TARGET_TIMEOUT_CPP_SEC: "5400"' "$extended_workflow"
grep -Fq 'HXHX_GATE3_TARGET_TIMEOUT_SEC: "4200"' "$m7_workflow"
grep -Fq 'HXHX_GATE3_TARGET_TIMEOUT_SEC=4200' "$testing_doc"
grep -Fq 'HXHX_GATE3_TARGET_TIMEOUT_<TARGET>_SEC' "$testing_doc"
grep -Fq 'HXHX_GATE3_TARGET_TIMEOUT_CPP_SEC=5400' "$testing_doc"
grep -Fq 'scripts/hxhx/check-local-capacity.js' "$runner"
grep -Fq 'HXHX_HEAVY_RUN_CAPACITY_POLICY' "$runner"
grep -Fq 'HXHX_LOCAL_CAPACITY:' "$testing_doc"
