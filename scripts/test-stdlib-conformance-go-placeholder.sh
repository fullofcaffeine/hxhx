...
OUT_REPORT="$TMP_DIR/go-placeholder-report.json"
...
if ! grep -q "PORTABLE_CONFORMANCE_RUNNER_START target=go tier=tier1" "$stdout_capture"; then
	echo "missing runner start marker" >&2
	exit 1
fi

if ! grep -q "PORTABLE_CONFORMANCE_RUNNER_SKIP target=go tier=tier1 reason=placeholder_not_implemented_in_this_repo" "$stdout_capture"; then
...
assertEqual(report.targetId, 'go', 'targetId')
...
echo "✓ go conformance placeholder adapter OK"
