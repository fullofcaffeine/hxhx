...
if ! command -v node >/dev/null 2>&1; then
	echo "Missing node on PATH (required for js-native KPI lanes)." >&2
	exit 1
fi
...
if ! target_available "js-native"; then
	echo "hxhx binary does not expose --target js-native (required for plugin/builtin KPI lanes)." >&2
	exit 1
fi
...
portable_cmd="\"$HXHX_BIN\" --target ocaml-stage3 --hxhx-no-emit -cp \"$WORK_DIR/src\" -main Main --hxhx-out \"$WORK_DIR/out_ocaml_portable\" -D ocaml_profile=portable"
