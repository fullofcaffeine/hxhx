#!/usr/bin/env node
/**
 * Cooperative ownership for heavyweight local Haxe-family compiler gates.
 *
 * The lease serializes only repositories that opt into this protocol. It
 * records the owning shell's PID and process start identity so a reused PID is
 * never mistaken for the original owner. Stale recovery removes the lease
 * file only; this module never signals or terminates another process.
 */
...
function defaultLeasePath(env = process.env) {
  if (env.HXHX_HEAVY_RUN_LEASE_FILE) return path.resolve(env.HXHX_HEAVY_RUN_LEASE_FILE)
  const user =
...
function readLeaseSnapshot(leasePath) {
  try {
    ...
    if (!before.isFile()) {
      return { status: 'incompatible', reason: 'not_regular_file', stat: before }
    }
...
function inspectLease(leasePath, options = {}) {
  ...
  if (snapshot.status === 'malformed') {
    return {
      status: heartbeatAgeMs >= staleAfterMs ? 'stale' : 'busy',
      reason: heartbeatAgeMs >= staleAfterMs ? 'malformed_expired' : 'lease_initializing',
      heartbeatAgeMs,
      snapshot,
    }
  }
  ...
  if (!validLeaseRecord(record)) {
    ...
    return {
      status: heartbeatAgeMs >= staleAfterMs ? 'stale' : 'busy',
      reason: heartbeatAgeMs >= staleAfterMs ? 'invalid_record_expired' : 'lease_initializing',
      heartbeatAgeMs,
      snapshot,
    }
  }
  ...
  if (heartbeatAgeMs >= staleAfterMs) {
    return { status: 'stale', reason: 'owner_unverifiable_expired', record, identity, heartbeatAgeMs, snapshot }
  }
...
function acquireLease({
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (
      inspection.status === 'busy' &&
      inspection.record &&
      sameOwner(inspection.record, ownerIdentity)
    ) {
...
function leaseSummary(result) {
  ...
  return {
    ...
    heartbeatAgeMs:
      result.inspection && Number.isFinite(result.inspection.heartbeatAgeMs)
        ? Math.round(result.inspection.heartbeatAgeMs)
        : 0,
  }
}
