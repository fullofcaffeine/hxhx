...
# Gate 1 (upstream unit macro suite) via stage0 shim.
#
# Why
# - This validates the upstream harness wiring and our OCaml "interp-like" build/run plumbing.
# - It is also a stable baseline while the non-delegating `hxhx` compiler pipeline is still in bring-up.
#
# Note
# - This invokes the system `haxe` binary (stage0). Use `run-upstream-unit-macro.sh` for the native/non-delegating
#   attempt.
...
if [ ! -d "$UPSTREAM_DIR/tests/unit" ]; then
  echo "Skipping upstream Gate 1 (stage0 shim): missing upstream Haxe repo at '$UPSTREAM_DIR'." >&2
...
if ! command -v dune >/dev/null 2>&1 || ! command -v ocamlc >/dev/null 2>&1; then
  echo "Skipping upstream Gate 1 (stage0 shim): dune/ocamlc not found on PATH."
  exit 0
fi
...
echo "== Gate 1 (stage0 shim): upstream tests/unit/compile-macro.hxml"
(
  cd "$UPSTREAM_DIR/tests/unit"
  HAXE_BIN="$HAXE_BIN" "$HXHX_BIN" -- compile-macro.hxml
)
