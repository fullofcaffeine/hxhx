...
compile_one() {
  ...
  # The source-to-runtime reports likewise have focused semantic, catalog, and
  # repeat-build checks. Copying them into every syntax fixture would mostly
  # duplicate runtime-selection evidence rather than test target code shape.
  diff -ru \
    --exclude '_GeneratedFiles.json' \
    --exclude 'ocaml_artifact_manifest.json' \
    --exclude 'ocaml_runtime_requirement_report.json' \
    --exclude 'ocaml_runtime_selection_shadow_report.json' \
