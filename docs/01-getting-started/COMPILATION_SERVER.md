...
- `reflaxe.ocaml` builds in this project currently start a fresh Haxe process
  ...
  source-generation tests, but the wider edit matrix and Dune-cache ownership
  are not complete enough for a supported persistent workflow.
...
### Experimental Reflaxe output transaction
...
This define is not a recommended normal-build setting yet. The OCaml output
directory also contains Dune's `_build` cache, and complete directory
replacement would make that cache cold on every request. The current experiment
therefore uses `ocaml_no_build`. Before the transaction becomes a default
workflow, Dune's reusable artifacts need a separate owned location and the
remaining edit, failure, memory, and timing matrix must pass.
