...
- Before Gate 3 resolves packages, builds a compiler, or creates its temporary upstream worktree, `scripts/hxhx/check-local-capacity.js` prints `HXHX_LOCAL_CAPACITY:<PASS|WARNING|BLOCKED|OFF>`. With the default `HXHX_HEAVY_RUN_CAPACITY_POLICY=auto`, sustained CPU load or reliable available memory below the larger of 4 GiB and 10% of total memory stops a local run with retryable exit code `75`, while CI records a warning and continues. macOS uses `memory_pressure`, Linux uses `MemAvailable`, and Windows uses available physical memory; raw free pages are diagnostic only when they are not a trustworthy availability signal. Use `require` to enforce the check everywhere, `warn` to observe it without stopping, or `off` to deliberately accept the slowdown. `HXHX_HEAVY_RUN_MAX_LOAD_PER_CPU`, `HXHX_HEAVY_RUN_MIN_AVAILABLE_MEMORY_GIB`, and `HXHX_HEAVY_RUN_MIN_AVAILABLE_MEMORY_FRACTION` change only capacity thresholds, while `HXHX_HEAVY_RUN_CAPACITY_REPORT=/path/report.json` retains a redacted JSON report. Set `HXHX_HEAVY_RUN_WAIT_SECONDS=900` to keep one attached Gate 3 command queued for up to 15 minutes. A queued local gate also holds the user-scoped cooperative Haxe-family lease; it reports only blocking-state changes, never kills foreign work, safely recovers stale owner identities, and releases its own lease on success, failure, or shell exit. Nested participating gates reuse the outer owner, while CI never acquires the lease. These controls never change targets, retries, timeouts, stage0 rules, or pass criteria.
