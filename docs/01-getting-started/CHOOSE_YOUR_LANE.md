...
| I want native binaries now, even if some work still delegates | `hxhx` compat lane (`--target ocaml-compat` or `--target js-compat`) | `"$(bash scripts/hxhx/build-hxhx.sh)" --target ocaml-compat -cp src -main Main` | `docs/01-getting-started/QUICKSTART_COMPAT.md` |
| I want strict non-delegating behavior checks | `hxhx` native lane (`--target ocaml` / `--target js`) with stage0 forbidden | `HXHX_FORBID_STAGE0=1 "$(bash scripts/hxhx/build-hxhx.sh)" --target ocaml --hxhx-no-emit -cp src -main Main` | `docs/01-getting-started/QUICKSTART_NATIVE.md` |
...
- **compat lane**: `--target *-compat` presets; compatibility-first and may delegate to stage0.
- **native lane**: `--target ocaml` / `--target js`; linked Stage3 path.
