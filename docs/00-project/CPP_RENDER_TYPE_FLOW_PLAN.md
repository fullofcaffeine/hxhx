...
## 2026-07-11 Scope-Isolated EReg Capture Callbacks

Follow-up bead `haxe_ocaml-thvk6` decomposed typed and fresh `EReg.map`
equality rendering with selectable fresh-process phases for inference,
receiver rendering, String adaptation, callback adaptation, exact map
rendering, comparable-argument rendering, optional probing, and typed-local
dispatch. The new probes retain exact full-call output and independently time
one named phase while all other assertions execute only once.

The dominant simple-map cost was callback adaptation, not receiver or result
inference. With only the candidate branch disabled but the same helper and
source layout present, three 5,000-call samples had medians of 0.884249s for
the typed callback, 0.959607s for typed map rendering, and 1.197377s for the
complete typed equality. Fresh equivalents were 0.847245s, 0.909274s, and
1.070222s. Receiver, literal String argument, optional-result probe, and
target-owned map predicate timings were much smaller.

The existing scope-isolated EReg callback renderer accepted exact
concatenations but declined a single callback-owned capture. It now also
accepts a String literal, zero-argument `matchedLeft`/`matchedRight`, a
literal-indexed `matched`, or the existing literal-indexed matched substring.
These shapes render before lambda-local scope copying. Dynamic indices,
captured values, other receivers, wrong arities, and every other callback body
still use the general lambda path. Nullable indexed captures preserve
`.value_or(std::string())`; side captures remain non-nullable Strings.

Three 5,000-call post-change samples had medians of 0.154169s for typed
callback adaptation, 0.233324s for typed map rendering, and 0.573383s for the
complete typed equality, reductions of about 82.6%, 75.7%, and 52.1%.
Fresh callback, map-render, and complete-equality medians were 0.118218s,
0.198128s, and 0.508186s, reductions of about 86.0%, 78.2%, and 52.5%.
Focused controls freeze exact side and nullable indexed output plus declines
for dynamic indices and non-callback receivers.

The exact-command current-source strict probe rendered all 88
`TestEReg.test` statements and reached the same `Parser_XmlParserException`
frontier before the expected 480-second timeout. The method moved from
0.032882s to 0.032201s and its class from 0.034571s to 0.033852s, about 2.1%
lower, while `TestBytes.test` moved from 1.168548s to 1.147620s. That aggregate
movement is not attributed to this shortcut: the 20 traced map statements
were effectively flat at 0.007719s versus 0.007548s, and their first-argument
render phases show that the 14 typed maps already use a cheap named callback.
The six fresh maps use different inline callback bodies. Follow-up
`haxe_ocaml-1upd7` owns those exact bodies and must use lambda-phase evidence
rather than infer a win from the method aggregate.
...
- three branch-disabled and three post-change 5,000-call selected samples
- `npm run hxhx:build-current-source`
- the exact-command non-delegating 480-second strict Cpp probe
- `npm run test:m14:cpp-native-backend-smoke`
- `npm run test:m14:cpp-helper-render-bench`
...
README Goals and North Star progress bars remain unchanged. This is a bounded
internal callback-render improvement while strict Cpp remains expected-red,
so public production usability does not change. Generated C++ is frozen by
focused and native controls; runtime support and committed bootstrap snapshots
are unchanged. `CppTargetCore.hx` remains a red mega-file at 26,173 lines; the
23-line production increase extends its existing target-owned EReg callback
seam, while broader extraction remains owned by `haxe_ocaml-36ec`. No upstream
compiler or test source was copied or consulted. `thinking:xhigh` was not
crossed. GPT 5.5 Pro and Oracle were deliberately skipped because exact
...
## 2026-07-12 Resolved EReg Map Callback Preparation

Follow-up bead `haxe_ocaml-f4lm9` split the remaining dynamic-local collector
around its late explicit callback declaration. Across independent 5,000-call
processes, the pre-change medians were 0.013388s for scope-map snapshots,
1.023062s for the 68-statement prefix, 0.355049s for the callback declaration,
2.102051s for the post-candidate statements, and 3.540783s for the complete
pass. The prefix registered 11 explicit EReg locals without producing a
candidate; the callback declaration produced the exact declared callable type;
and the post-candidate phase produced the exact callback override.

The 19 later EReg map expressions all supply the same already-typed local
callback. The first map establishes the fixed target-owned EReg callback type
in both the local and argument override maps. Later maps previously repeated
the same EReg receiver and callback preparation even though both maps already
agreed. The collector now declines that repeated work only for a direct typed
local EReg `map` call with two arguments, an explicit callback candidate, and
both existing overrides equal to the exact EReg callback type. Missing,
conflicting, stale, open-Dynamic, non-EReg, nested, and general known-parameter
evidence still takes the existing paths.

The same-source old-path replay measured 0.072826s versus 0.043766s for the
current already-resolved forwarded-map path, about 39.9% lower. The complete
pass moved from 3.540783s to 2.854434s, about 19.4%, and post-candidate
collection moved from 2.102051s to 1.491344s, about 29.1%. Prefix collection
was effectively flat at 1.017146s versus 1.023062s, as was callback declaration
at 0.351865s versus 0.355049s. The no-forward control measured 1.430209s versus
1.474469s, a small movement not used as causal evidence. Every selection keeps
the exact candidate, local-type, and override assertions active.

The exact-command current-source strict probe cloned `hxcpp` in 74 seconds,
rendered all 88 TestEReg statements, and reached the same
`Parser_XmlParserException` frontier before the expected 480-second timeout.
`infer_dynamic_locals` moved from 0.001119s to 0.001027s, about 8.2%, and the
cache-miss preparation total moved from 0.002942s to 0.002789s. The method,
class, cache hit, 88-statement sum, and independent `TestBytes.test` control
moved slightly in mixed directions, so no aggregate claim is made. All 484
selected records were present.

Follow-up `haxe_ocaml-to5wc` owns attribution of the now-stable 1.017146s
prefix, especially repeated local-type discovery and registration for its 11
ordinary EReg locals, before another production shortcut is considered.

Relevant evidence is:

- `.artifacts/full1/cpp-strict-current/cpp-dynamic-local-candidate-collection-selected.log`
- `.artifacts/full1/cpp-strict-current/cpp-dynamic-local-resolved-forwarded-map-baseline.log`
- `.artifacts/full1/cpp-strict-current/cpp-dynamic-local-resolved-forwarded-map-after-cache.log`
- `.artifacts/full1/cpp-strict-current/gate3-cpp-testereg-after-direct-dynamic-local-candidate-gate.log`
- `.artifacts/full1/cpp-strict-current/gate3-cpp-testereg-after-resolved-ereg-map-callback-cache.log`

Focused validation for this slice includes:

- three independent 5,000-call samples for each selected phase and same-source
  current/old already-resolved map paths
- `npm run test:m14:cpp-native-backend-smoke`
- `npm run test:m14:cpp-helper-render-bench`
- `npm run hxhx:build-current-source`
- the exact-command non-delegating strict Cpp probe
...
internal preparation improvement while strict Cpp remains expected-red, so
public production usability does not change. `CppTargetCore.hx` remains a red
mega-file at 26,212 lines; the 21-line production increase stays beside the
existing target-owned EReg preparation seam, while broader extraction remains
owned by `haxe_ocaml-36ec`. Selectable attribution stays in the dedicated
documented 357-line bench module. The ignored upstream suite was executed only
as a behavior oracle; no upstream compiler or test source was copied.
`thinking:xhigh` was not crossed. GPT 5.5 Pro and Oracle were deliberately
...
## 2026-07-12 Optional-Lambda Preparation Attribution

Follow-up bead `haxe_ocaml-2lv4j` decomposed TestEReg optional-lambda local
preparation without a production edit. The production branch first checks the
exact target-known method exclusions, neither of which applies to this phase,
then uses a syntax-only guard for unhinted parser-normalized optional-lambda
locals before entering call-shape and function-type inference.

Across three fresh selectable 5,000-call processes, the target-known skip
median was 0.034195s, the evidence-guard median was 0.077469s, and the complete
negative branch median was 0.111940s. The sum of the two required checks is
within about 0.25% of the complete branch, leaving no distinct inference seam
after them. The complete positive branch median was 0.613277s and retained the
exact `std::function<int(int, std::optional<int>)>` override.

Exact controls cover unhinted and explicitly typed normalized optional
lambdas, ordinary and malformed lambda/helper exclusion, nested inference, a
typed outer shadow with distinct nested local naming, unrelated locals, the
non-applicability of the existing `Unserializer.unserialize` and
`TestType.testInlineCast` skips, negative no-write state, and exact positive
override state. No compiler source changed, so no current-source rebuild or
strict rerun is required.

Follow-up `haxe_ocaml-1kt87` owns separate attribution of the next unowned
`infer_callable_args` phase, about 0.000028s in the latest strict trace. The
larger bind, dynamic-local, and helper-typed-as phases already have separate
completed attribution.

Relevant evidence is:

- `.artifacts/full1/cpp-strict-current/cpp-optional-lambda-prep-attribution.log`
- `.artifacts/full1/cpp-strict-current/gate3-cpp-testereg-after-bind-candidate-gate.log`

Validation for this attribution-only slice includes three independent 5,000-
call target-known-skip, guard, complete-negative, and complete-positive
selections; exact normalized, typed, nesting, shadow, malformed, unrelated,
skip, and override controls; the Cpp native smoke and full helper-render bench;
the Cpp plan, mega-file, changed/full Haxe format guards; and
`git diff --check`.
...
readiness. `CppTargetCore.hx` remains unchanged at 26,254 lines, extraction
remains owned by `haxe_ocaml-36ec`, and attribution is isolated in a documented
166-line bench module. No upstream source was copied. `thinking:xhigh` was not
crossed; Oracle/GPT were skipped because the existing skip and guard fully
account for the negative phase.
