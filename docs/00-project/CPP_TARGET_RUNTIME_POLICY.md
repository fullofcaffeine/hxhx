...
| Payload enum constructors | Constructor payloads are observable through `Std.string`, switch binders, equality, Dynamic, and reflection APIs. | Typed enum value contexts now preserve two payload views on `std::shared_ptr<Enum>` carriers: string metadata for `Std.string` / focused switch binders, and original `std::any` payloads for equality and `Type.enumParameters`. Raw constructor helper methods, broad erased switch extraction, and Serializer parity are still incomplete. | Continue as bounded bring-up. Do not optimize payload constructors as pure tag methods. |
