...
The current reviewed baseline contains 320 legacy entries:

| Domain | Entries | What it means |
| --- | ---: | --- |
| Structured expressions | 301 | Direct `EIdent` or `EField(EIdent)` construction of a private runtime symbol. |
...
Most entries—304—are in `OcamlBuilder.hx`. That is an ownership warning, not a
