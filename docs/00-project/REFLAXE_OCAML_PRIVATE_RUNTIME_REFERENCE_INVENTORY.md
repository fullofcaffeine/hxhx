...
The current reviewed baseline contains 388 legacy entries:

| Domain | Entries | What it means |
| --- | ---: | --- |
| Structured expressions | 358 | Direct `EIdent` or `EField(EIdent)` construction of a private runtime symbol. |
...
Most entries—371—are in `OcamlBuilder.hx`. That is an ownership warning, not a
