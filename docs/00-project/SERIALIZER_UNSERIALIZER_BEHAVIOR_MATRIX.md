...
| Cpp constructor-name enum round trips | `bounded_bringup_support`, `review_required` before expansion | `haxe_ocaml-73r4j` adds target-owned Cpp `Serializer` / `Unserializer` support for the focused enum carrier case: `w` constructor-name encoding, enum name, constructor tag, payload order, and original `std::any` payloads are preserved for the zero-arg and `Int`/`String` payload smoke. This is not full Serializer parity. |
| Cpp `Unserializer.unserializeEnum` compact helper | `bounded_bringup_support`, `review_required` | Cpp now has a bounded runtime path for name-mode enum tokens that recursively decodes `null`, `Bool`, `Int`, `String`, and enum payloads into `Type.createEnum<EnumValue>`. Enum index mode, references, custom resolvers, object/class payloads, arrays/maps, bytes, floats, string cache/escaping breadth, and custom hooks remain outside this support. |
