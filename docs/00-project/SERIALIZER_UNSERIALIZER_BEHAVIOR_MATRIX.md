...
| Cpp enum round trips | `bounded_bringup_support`, `review_required` before expansion | `haxe_ocaml-73r4j` added target-owned Cpp `Serializer` / `Unserializer` support for focused enum carrier `w` constructor-name encoding. `haxe_ocaml-nd2nh` extends that to `j` constructor-index encoding through `Serializer.USE_ENUM_INDEX`. In both modes, enum name, constructor identity, payload order, and original `std::any` payloads are preserved for the zero-arg and `Int`/`String` payload smokes. This is not full Serializer parity. |
| Cpp `Unserializer.unserializeEnum` compact helper | `bounded_bringup_support`, `review_required` | Cpp now has bounded runtime paths for name-mode and index-mode enum tokens that recursively decode `null`, `Bool`, `Int`, `String`, and enum payloads into `Type.createEnum<EnumValue>` / `Type.createEnumIndex<EnumValue>`. References, custom resolvers, object/class payloads, arrays/maps, bytes, floats, string cache/escaping breadth, and custom hooks remain outside this support. |
...
| `ser-enum-01` | zero-arg and arg enums | Pin constructor names, arg order, value preservation, and decoded enum identity. | Name-mode Cpp smoke coverage now passes for focused zero-arg and `Int`/`String` payload enum carriers. Any broader payload type or resolver behavior still needs explicit oracle-backed classification. |
| `ser-enum-index-01` | `Serializer.USE_ENUM_INDEX` | Pin indexed output and decoded enum behavior. | Focused Cpp smoke coverage now passes for zero-arg and `Int`/`String` payload enum carriers using upstream-compatible `j` tokens. Any broader payload type or resolver behavior still needs explicit oracle-backed classification. |
