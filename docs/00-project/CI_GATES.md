...
| `Macro Runtime Parity (Weekly)` | `.github/workflows/macro-runtime-parity-weekly.yml` | Runs upstream macro + display checks in both macro runtime modes (`external-host`, `inproc`). It also builds one real Haxe-authored project macro as authenticated bytecode/native plugins and runs it through both modes. The Full1 marker is emitted only when the two-mode matrix and this project proof all pass. | **Nightly/scheduled + Release + Reusable** | weekly schedule, manual, `release`, `workflow_call` |
