...
| `Reflaxe OCaml / Package Artifact Matrix` | `.github/workflows/reflaxe-ocaml-package-matrix.yml` | Builds one deterministic source ZIP, passes that exact artifact to Ubuntu and macOS install/native-run consumers, then opens both receipts and requires the same clean commit and package SHA before emitting `RO_PACKAGE_ARTIFACT_MATRIX:PASS`. | **Nightly/scheduled product evidence** | relevant pushes/PRs, weekly schedule, manual |
