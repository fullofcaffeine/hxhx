...
| Gate M7 / Replacement Bundle | `.github/workflows/gate-m7.yml` | Weekly schedule | `M7_SHARED_ARTIFACTS:PASS`, `M7_STRICT_STAGE0:PASS`, and `M7_REPLACEMENT_READY:PASS` | Artifact `gate-m7-logs-<run_id>` with the run log and `m7-shared-artifacts.v1` receipt + run logs |
