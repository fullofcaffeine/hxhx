...
- In particular, the current external-host runtime subset is still narrower than the vendored `reflaxe-elixir` usage of:
  - `Context.parseInlineString()` / `Context.typeExpr()` in inline markup / HEEx TSX flows
