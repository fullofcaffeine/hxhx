...
| --- | --- | --- | --- |
| MRP-B2 | Macro API surface is still incomplete (`haxe.macro.Context` / `haxe.macro.Compiler` methods are partially implemented in bring-up layers). Recent slices now cover `Compiler.getConfiguration()`, `Context.currentPos()`, `Context.getDisplayMode()`, `Context.getPosInfos()`, and `Context.makePosition()` in the external-host runtime, but typed reflection/query APIs remain open. | Upstream macro compatibility claims are limited until API coverage expands. | `haxe.ocaml-bxlg.9.5` |
