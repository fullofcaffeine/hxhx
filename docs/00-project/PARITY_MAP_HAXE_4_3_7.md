...
| PM-13 | Macro runtime parity matrix (unit macro + runci macro + display/protocol checks + one Haxe-authored project macro) | Native Stage3 macro runtime parity | `Macro` | runtime mode matrix (`external-host`,`inproc`) plus authenticated project plugin | `.github/workflows/macro-runtime-parity-weekly.yml` (standalone + reusable inside `.github/workflows/gate-full1.yml`) | `MACRO_RUNTIME_EXTERNAL_HOST_ARTIFACT:PASS`, `PROJECT_MACRO_MODULE:PASS`, `MACRO_RUNTIME_PARITY_UNIT_EXTERNAL_HOST:PASS`, `MACRO_RUNTIME_PARITY_UNIT_INPROC:PASS`, `MACRO_RUNTIME_PARITY_RUNCI_EXTERNAL_HOST:PASS`, `MACRO_RUNTIME_PARITY_RUNCI_INPROC:PASS`, `MACRO_RUNTIME_PARITY_DISPLAY_PROTOCOL_EXTERNAL_HOST:PASS`, `MACRO_RUNTIME_PARITY_DISPLAY_PROTOCOL_INPROC:PASS`, `MACRO_RUNTIME_PARITY_EXTERNAL_HOST:PASS`, `MACRO_RUNTIME_PARITY_INPROC:PASS`, `MACRO_RUNTIME_PARITY_WEEKLY:PASS`, `FULL1_MACRO_PARITY:PASS` |
